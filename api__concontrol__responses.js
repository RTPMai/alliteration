// PUT IN: api/concontrol/responses.js
...
import {
  newResponse, newSignup, rowsFromCsv, responseKey, summarise, signupIsUsable, manualSignup, signupFromRow,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // Somebody who never touched the form: a booth conversation, a phone
      // call, a business card. Same dedupe as the import, so they cannot end
      // up on the list twice whichever way they arrived.
      if (what === "add-signup") {
        const out = manualSignup(b, await listSignups(event));
        if (out.error) return res.status(out.duplicate ? 409 : 400).json({ error: out.error, duplicate: out.duplicate || null });
        const id = await nextSignupId();
        const record = newSignup(id, event, out.answers, "manual");
        record.history = [historyEntry("added by hand", sess.username)];
        await saveSignup(record);
        return res.status(201).json({ ok: true, signup: record });
      }
