...
//                                spends with us. Folds merged clients together
//                                when the caller supplies a mergeOf map, so a
//                                company held as two Printavo records is one
//                                line rather than two half-sized ones.
...
export function summarise(requests, opts) {
  ...
  // absorbedId -> { primaryId, name }, supplied by the caller. Empty when
  // nothing has been merged, which is the normal case.
  const mergeOf = options.mergeOf || {};
  ...
  rows.forEach((row) => {
    ...
    //
    // MERGED CLIENTS COUNT ONCE HERE TOO. A request matched to a record that
    // has since been merged away still names that record, so without this the
    // client table would show one company on two lines with its giving split
    // between them, which is the same splitting the merge exists to end.
    const acct = row.account || {};
    if (acct.found && acct.customerId) {
      const merged = mergeOf[String(acct.customerId)];
      const key = merged ? String(merged.primaryId) : String(acct.customerId);
      if (!byCustomer[key]) {
        byCustomer[key] = {
          customerId: key,
          name: (merged && merged.name) || acct.name || row.request.orgName || "",
          tier: acct.tier || null,
          retail: 0,
          cost: 0,
          count: 0,
