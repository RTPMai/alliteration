// PUT IN: lib/notifications/store.js
...
//
//   { id, assignedTo, createdBy, status, visibility, triggerDate }
//
...
//
// triggerDate joined in Sep 2026 with reminders: a reminder before its day
// must not light up the rail count, and the count never reads a record.
// An older summary without the field is correct as-is, because nothing
// written before reminders existed has a trigger date.
//
// THIS IS A SECOND COPY OF SIX FACTS, and two copies of one fact drift.
...
import { keys, isWaiting, todayInZone } from "./schema.js";
...
/**
 * The six fields the index carries, derived from a record.
 *
...
export function summaryOf(record) {
  ...
  return {
    ...
    triggerDate: record.triggerDate || null,
  };
}
 ...
 */
export function countSummaries(summaries, filters, me, today) {
  const who = String(me || "").toLowerCase();
  const f = filters || {};
  // "Open" means open NOW. A reminder waiting for its day is not something
  // to do yet, so the rail does not count it until the morning it arrives.
  // today defaults to the shop's calendar; tests pass a fixed day.
  const day = today || todayInZone();
  return summaries.filter((s) => {
    if (s.visibility === "private" && s.createdBy !== who) return false;
    if (f.status === "open" && isWaiting(s, day)) return false;
