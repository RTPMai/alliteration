...
// Which SCREEN inside that app a link opens on. Moved here from
// apps/notifications.js (Sep 24 2026) when the Today screen started opening
// the same links: two copies of "an inquiry opens on Inquiries" is how one of
// them ends up pointing at a screen that was renamed. "client" has no
// top-level BackBone view of its own, so it opens the dashboard and
// BackBone's own deep-link handler switches to the roster sub-tab.
export const LINK_TYPE_VIEW = {
  inquiry: "inquiries", lead: "inquiries", client: "dashboard",
  expense: "expenses", donation: "requests",
  po: "orders", sticky: "board",
  timeoff: "timeoff",
};

/** Where a stored link opens: { app, view }, or null for an unknown type. */
export function linkRoute(type) {
  const app = LINK_TYPE_APP[type];
  const view = LINK_TYPE_VIEW[type];
  return app && view ? { app, view } : null;
}
