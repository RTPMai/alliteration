// PUT IN: lib/notifications/schema.js (REPLACES the current one)
...
// A notification can point at the specific BackBone record it's about: an
// inquiry (BackBone Inbox), a lead (the leads pipeline), or a client (the
// roster). Optional — plenty of notifications ("restock the coffee") have
// nothing to link to. Auto-created notifications (lead handoff, inquiry
// routing, apps/backbone/main.js) always set this; a person creating one
// by hand can also search and pick a record via the same three types.
export const LINK_TYPES = ["inquiry", "lead", "client"];
export const LINK_TYPE_LABELS = { inquiry: "Inquiry", lead: "Lead", client: "Client" };
...
export function validateNew(body, appIds, assignableUsernames) {
  ...
  return {
    ok: true,
    errors: [],
    record: { title, types, appIds: appTags, assignedTo, dueDate, link },
  };
}
