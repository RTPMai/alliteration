// PUT IN: lib/notifications/filters.js
...
import { isWaiting } from "./schema.js";
...
export const STATUS_FILTERS = [
  ...
  // Reminders that have not reached their day (Sep 2026). "Open" hides
  // them, so without this there would be no way to see or fix one you set.
...
export function matchesFilters(n, raw, today) {
  ...
  const waiting = isWaiting(n, day);
  if (f.status === "open" && (status !== "open" || waiting)) return false;
  if (f.status === "scheduled" && !waiting) return false;
  if (f.status === "done" && status !== "done") return false;
...
/**
 * How many reminders in a list are still waiting for their day. Drives the
 * "2 scheduled reminders" shortcut in the count line, so a reminder somebody
 * just set does not look like it vanished.
 */
export function countWaiting(list, today) {
  const day = str(today) || todayLocalISO();
  return (Array.isArray(list) ? list : []).filter((n) => isWaiting(n, day)).length;
}

/**
 * Filter and order a list. Open items come first, then reminders still
 * waiting (soonest trigger first), then done. Within a group, the nearest
 * due date, then newest: a dated item is a promise to somebody and outranks
 * an undated one no matter which was typed first.
 */
export function applyFilters(list, raw, opts) {
  const o = opts || {};
  const day = str(o.today) || todayLocalISO();
  const src = Array.isArray(list) ? list : [];
  const rank = (n) => ((n.status || "open") === "done" ? 2 : isWaiting(n, day) ? 1 : 0);
  return src
    .filter((n) => matchesFilters(n, raw, day))
    .sort((a, b) => {
      const ar = rank(a), br = rank(b);
      if (ar !== br) return ar - br;
      if (ar === 1 && a.triggerDate !== b.triggerDate) {
        return String(a.triggerDate) < String(b.triggerDate) ? -1 : 1;
      }
