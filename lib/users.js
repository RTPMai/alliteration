...
export const DEFAULT_ROLES = {
  ...
  // who need to see their own PTO balance and request time off, but nothing
  // else in the shell and no other employee's data. data_scope "own" is what
  // api/crewcore/*.js reads to gate a caller down to their own employee
  // record — see lib/crewcore/schema.js's ADMIN_ONLY_FIELDS for the fields
  // stripped even from that own-record view.
  //
  ...
  employee: {
    ...
    protected: false,
    apps: ["crewcore"],
    tabs: ["crewcore:dashboard", "crewcore:roster", "crewcore:pto", "crewcore:reviews"],
    data_scope: "own",
    can_edit: false,
    can_export: false,
    manage_lists: false,
...
export async function permsFor(username) {
  ...
  const viewGrants = Array.isArray(role.tabs) ? role.tabs.slice() : [];
  return {
