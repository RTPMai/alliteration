...
/**
 * The MarketMachine views somebody who is not an Admin may reach: My tasks,
 * and nothing else. Sept 2026, when the app was opened to the people doing the
 * work. js/registry.js holds the same list for the client-side half.
 *
 * A ceiling, not a grant: ticking MarketMachine on an account is what gives
 * somebody their tasks, and it must never also hand them campaign pages,
 * budgets or Settings. Only the per-account Admin flag lifts it.
 */
const MARKETMACHINE_SELF_VIEWS = ["marketmachine:tasks"];
...
export const DEFAULT_ROLES = {
  ...
  // view. Time off came back Sep 21 2026 as "crewcore:timeoff" (requests
  // and balances); the old Aug 2026 "crewcore:pto" key stays retired.
  //
  ...
  employee: {
    ...
    tabs: ["crewcore:dashboard", "crewcore:timeclock", "crewcore:timeoff", "crewcore:stipend", "crewcore:samples", "crewcore:kudos", "crewcore:reviews", "crewcore:handbook", "stitchsense:guess"],
    data_scope: "own",
    can_edit: false,
    can_export: false,
    manage_lists: false,
...
export async function permsFor(username) {
  ...
  if (appGrants.includes("crewcore") && !isAdmin) {
    ...
    // TIME OFF, Sep 21 2026: everybody with CrewCore gets it, even when their
    // views were narrowed by hand before the tab existed. Requesting a day
    // off is the one thing every employee needs, and an account ticked for
    // "Dashboard, Stipend, Reviews, Handbook" in August would otherwise never
    // see it until somebody remembered to tick one more box per person.
    if (!viewGrants.includes("crewcore:timeoff")) viewGrants.push("crewcore:timeoff");
  }

  // MARKETMACHINE, Sept 2026: same rule, same reason. The campaign screens are
  // Admin only in api/marketmachine/campaigns.js, so without this the rail
  // would offer screens the server refuses, which reads as a broken app rather
  // than as a boundary.
  if (appGrants.includes("marketmachine") && !isAdmin) {
    // Exactly what was ticked for this person, and My tasks when nothing was.
    // Sept 2026: ticking Campaigns is how Jacob gets the whole app without the
    // platform Admin flag, which would also hand him pay, reviews and
    // accounts. See lib/marketmachine/access.js for the one definition.
    const kept = viewGrants.filter((t) => t.startsWith("marketmachine:"));
    viewGrants = viewGrants
      .filter((t) => !t.startsWith("marketmachine:"))
      .concat(kept.length ? kept : MARKETMACHINE_SELF_VIEWS.slice());
  }
