...
import { resolveGrants, asRole, normalizeGrants } from "./user-grants.js";
...
function publicUser(u) {
  return {
    ...
    role: u.role,
    // Per-account elevated flag, separate from role. Grants visibility into
    // stub/pre-release apps (see js/registry.js canAccess) that no role's
    // app list ever carries, e.g. CrewCore's employee pay/review data.
    superuser: u.superuser === true,
    // Per-account grant overrides, or null when this account is exactly its
    // role. Safe to send: these are the same switches the Roles screen
    // already shows, not anything about the password.
    grants: normalizeGrants(u.grants),
...
/**
 * ONE PERSON'S RESOLVED ACCESS, shaped like a role object.
 *
 * This is what every route should ask, and the reason it exists is a bug I
 * shipped: per-account grants landed in permsFor(), which the rail reads, but
 * eighteen routes were still calling getRole(user.role) directly. So a grant
 * set on somebody's account changed what they SAW and not what the server let
 * them DO. Buttons hidden, endpoints open.
 *
 * getRole() answers "what does this role say". getAccess() answers "what may
 * this person do", which is the question every one of those call sites was
 * actually asking.
 */
export async function getAccess(username) {
  const rec = await getUserRecord(username);
  if (!rec) return await getRole(null);
  const role = await getRole(rec.role);
  return asRole(resolveGrants(role, rec.grants), role);
}
...
export async function permsFor(username) {
  ...
  if (!rec) return { tabs: [] };
  const role = await getRole(rec.role);
  // Per-view scoping: a role's OWN "tabs" list (if present) carries scoped
  // "<appId>:<view>" entries — see the "employee" role's CrewCore comment.
  // Concatenated with apps rather than replacing them, since allowedViews()
  // treats an app id with no ":view" suffix as app-level access and only
  // narrows per-view when a "<appId>:" prefixed entry exists for that app.
  // The account's own grants beat the role's, key by key. An account with
  // none behaves exactly as it did before per-account grants existed.
  const resolved = resolveGrants(role, rec.grants);
  // Shaped like a role so lib/giving-access.js can answer its three
  // questions against the resolved values without knowing accounts exist.
  const effectiveRole = asRole(resolved, role);

  // APP-LEVEL ONLY. A "<app>:<view>" entry in an apps list would be
  // concatenated straight onto tabs below and would skip every ceiling
  // applied further down, because the ceilings filter viewGrants and this is
  // not viewGrants. That hole existed for role app lists too; per-account
  // grants would have made it reachable from the Accounts screen. Scoped
  // entries belong in `tabs`, which is owned by code.
  const appGrants = resolved.apps.filter((a) => !String(a).includes(":"));
  // `tabs` is NOT overridable per account. It carries scoped "<app>:<view>"
  // entries and is owned by code (see getRoles), so a per-account copy would
  // be the same stale-snapshot problem one level down.
  let viewGrants = Array.isArray(role.tabs) ? role.tabs.slice() : [];

  // CREWCORE IS DENY-BY-DEFAULT PER VIEW, Aug 28 2026.
  //
  // A role with no `tabs` list means "every view of every app you can open",
  // which is right for the other apps and was wrong for this one. Only the
  // self-serve "employee" role ships a tabs list, so ANY other role with
  // CrewCore ticked — including a role created in Settings for some
  // unrelated reason — got Roster, Time Clock and Settings in its rail. That
  // is exactly what happened to an office account on the first deploy of
  // this work: the server answered every request correctly with her own
  // record only, and the rail still offered her the whole team's roster.
  //
  // So the self-serve set is applied as a CEILING here, to anyone who is not
  // a CrewCore admin, whatever their role lists. isCrewCoreAdmin is the same
  // rule the routes use: the per-account Admin flag or the protected admin
  // role, never a role checkbox.
  if (appGrants.includes("crewcore") &&
      !isCrewCoreAdmin({ superuser: rec.superuser, roleName: rec.role })) {
  ...
  // punch — see lib/crewcore/schema.js), so the grant is narrowed here
  // rather than being a second thing to remember to switch off on a role.
  //
  // Only roles that carry a scoped crewcore grant pay for the lookup, which
  // in practice is the self-serve "employee" role. An admin has no scoped
  // tabs at all, so this costs them nothing on every call to permsFor().
  //
  ...
  return {
    ...
    manage_lists: resolved.manage_lists,
    ...
    can_delete_notifications: resolved.can_delete_notifications,
    ...
    // Accounts screen.
    superuser: rec.superuser === true,
    role: rec.role,
    // Which answers came from the account rather than the role. The Accounts
    // screen shows this so "why can she see that" has one place to look.
    grant_sources: resolved.sources,
  };
}
...
export async function updateUser(username, patch = {}) {
  ...
  // PER-ACCOUNT GRANTS. Passing null (or an object with nothing in it) resets
  // this person to their role rather than storing an empty object, so an
  // account that has been reset is indistinguishable from one that was never
  // touched. Unknown keys are dropped by normalizeGrants rather than stored
  // looking like settings that do nothing.
  if (patch.grants !== undefined) {
    const g = normalizeGrants(patch.grants);
    if (g) rec.grants = g; else delete rec.grants;
  }
