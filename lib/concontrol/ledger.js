...
export function budgetSummary(entries, sponsors, budget) {
  ...
  // ONLY CASH IS INCOME. A credit on our account with a supplier and a video
  // somebody shot for free are both worth real dollars, and neither one is in
  // the bank. Adding them here would produce a position you could quote to
  // somebody and be wrong about.
  const incomeIn = round(sponsorCash + otherIncome.received);
