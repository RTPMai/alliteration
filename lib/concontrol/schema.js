 ...
 *
 * AND ALL THREE CAN COVER THE BUDGET. A $7,000 credit with a supplier we were
 * going to spend $7,000 with anyway is worth exactly $7,000 to the event: the
 * money simply never leaves. Treating it as not counting showed a hole that was
 * not there.
 *
 * SO THE QUESTION IS NOT "WHAT KIND" BUT "WOULD WE HAVE SPENT IT ANYWAY". That
 * is a judgement about the deal, not a property of the payment, so it is its
 * own field: `offsets`. On by default for credit and in kind, because that is
 * the normal case, and turn-off-able for the one that is genuinely a nice to
 * have. A branded photo booth nobody would have bought is not budget relief;
 * the apparel we were buying regardless is.
 *
 * ONLY CASH IS CASH, still. Covering the budget and being in the bank are two
 * different questions and both get asked, so both numbers exist. What changed
 * is which one leads.
 */
 ...
 */
/**
 * Does this payment take money off what we were going to spend anyway?
 *
 * Cash always does, trivially. For credit and in kind the default is yes,
 * because that is the normal case, and an explicit false is the only thing
 * that turns it off. Absent means yes: every payment written before this field
 * existed was recorded on the understanding that it counted.
 */
export function paymentOffsets(payment) {
  const kind = pickOne(payment && payment.kind, PAYMENT_KINDS, DEFAULT_PAYMENT_KIND);
  if (kind === "cash") return true;
  return !(payment && payment.offsets === false);
}

export function paidByKind(sponsor) {
  const list = sponsor && Array.isArray(sponsor.payments) ? sponsor.payments : [];
  const out = { cash: 0, credit: 0, "in-kind": 0, covering: 0, extra: 0 };
  for (const p of list) {
    const amt = money(p && p.amount);
    if (amt === null) continue;
    const kind = pickOne(p && p.kind, PAYMENT_KINDS, DEFAULT_PAYMENT_KIND);
    out[kind] += amt;
    if (paymentOffsets(p)) out.covering += amt;
    else out.extra += amt;
  }
  out.covering = Math.round(out.covering * 100) / 100;
  out.extra = Math.round(out.extra * 100) / 100;
...
export function sponsorMoney(sponsor) {
  ...
  return {
    ...
    nonCash: kinds.value,
    // What this sponsor is worth to the budget: cash, plus anything that comes
    // off spend we were going to make anyway.
    covering: kinds.covering,
    // Value we are glad to have and were never going to buy. Real, and not
    // budget relief, so it is never added to a budget figure.
    extra: kinds.extra,
...
export function rollup(sponsors, today = new Date()) {
  ...
  return {
    ...
    // The number the budget runs on.
    covering: Math.round(covering * 100) / 100,
    ...
    if (!rows) errors.push("Payments must be a list");
      ...
      rows.forEach((p, i) => {
        ...
        clean.push({
          ...
          offsets: p && p.offsets === false ? false : true,
