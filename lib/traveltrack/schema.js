...
export function validateOrgSettings(input) {
  ...
  // Extra people who can be named on a trip but have no shell login of
  // their own. The attendee picker merges this with the real user list, so
  // the roster only needs to carry the difference — not everyone twice.
  if (input.team_members !== undefined) {
    let list = input.team_members;
    if (typeof list === "string") list = list.split(/[;,\n]/);
    if (!Array.isArray(list)) {
      errors.push("team_members must be a list of names");
    } else {
      const names = list.map((n) => String(n || "").trim()).filter(Boolean);
      const seen = new Set();
      rec.team_members = names.filter((n) => {
        const k = n.toLowerCase();
        if (seen.has(k)) return false;
        seen.add(k);
        return true;
