...
export const keys = {
  ...
  loyalty:         (id) => `${KEY_PREFIX}:loyalty:${id}`,
  loyaltyIndex:    () => `${KEY_PREFIX}:loyalty_index`,
...
export const TRIP_STATUSES = ["planned", "in_progress", "completed", "cancelled"];
...
export const LOYALTY_PROGRAM_TYPES = ["airline", "hotel", "rental_car", "other"];

export const LOYALTY_UNITS = ["miles", "points"];
...
export function validateLoyaltyAccount(input, { partial = false } = {}) {
  const errors = [];
  const rec = {};

  if (input.program_type !== undefined) {
    if (!LOYALTY_PROGRAM_TYPES.includes(input.program_type)) {
      errors.push(`program_type must be one of: ${LOYALTY_PROGRAM_TYPES.join(", ")}`);
    } else {
      rec.program_type = input.program_type;
    }
  } else if (!partial) {
    errors.push("Program type is required");
  }

  if (input.program_name !== undefined) rec.program_name = String(input.program_name || "").trim();
  else if (!partial) errors.push("Program name is required");
  if (!partial && !rec.program_name) errors.push("Program name is required");

  if (input.account_number !== undefined) rec.account_number = String(input.account_number || "").trim();

  if (input.balance !== undefined) {
    const balance = Number(input.balance);
    if (Number.isNaN(balance) || balance < 0) errors.push("balance must be a non-negative number");
    else rec.balance = balance;
  } else if (!partial) {
    rec.balance = 0;
  }

  if (input.balance_unit !== undefined) {
    if (!LOYALTY_UNITS.includes(input.balance_unit)) {
      errors.push(`balance_unit must be one of: ${LOYALTY_UNITS.join(", ")}`);
    } else {
      rec.balance_unit = input.balance_unit;
    }
  } else if (!partial) {
    rec.balance_unit = "miles";
  }
...
export function validateRedemption(input) {
  ...
  if (!input.description || !String(input.description).trim()) errors.push("Description is required");
  else rec.description = String(input.description).trim();

  const amount = Number(input.amount_redeemed);
  if (Number.isNaN(amount) || amount <= 0) errors.push("amount_redeemed must be a positive number");
  else rec.amount_redeemed = amount;

  if (input.est_value !== undefined && input.est_value !== "" && input.est_value !== null) {
    const v = Number(input.est_value);
    if (Number.isNaN(v) || v < 0) errors.push("est_value must be a non-negative number");
    else rec.est_value = round2(v);
  }
