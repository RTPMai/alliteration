...
// ---- Redeem Miles -----------------------------------------------------
// A running dollar total lives directly on the trip (trip.miles_value —
// see lib/traveltrack/schema.js). Adding a redemption increments that
// total and appends to a lightweight log (trip.miles_log) for an audit
// trail; every display surface only ever shows the cumulative total.

export async function addMilesRedemption(tripId, redemption, by) {
  const existing = await getTrip(tripId);
  if (!existing) return null;
  const log = Array.isArray(existing.miles_log) ? existing.miles_log.slice() : [];
  const entry = {
    id: "MR" + (log.length + 1),
    ...
    by: by.username,
    by_name: by.name || by.username,
  };
  log.push(entry);
  const newTotal = Math.round(((Number(existing.miles_value) || 0) + entry.amount) * 100) / 100;
  return updateTrip(tripId, { miles_value: newTotal, miles_log: log });
}
