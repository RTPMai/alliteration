...
import { demoCampaigns } from "./demo.js";
import {
...
/* ---- example campaigns, for showing the app to somebody ---------------- */

/**
 * Write the examples. Ids come from the same sequence as real campaigns, so
 * an example and a real campaign can never end up sharing one, and the
 * connected campaigns really point at the example event.
 */
export async function loadDemo(session, today) {
  const ids = await readIndex();
  const records = demoCampaigns(session, today);
  const assigned = [];
  const idByParentIndex = {};
  for (let i = 0; i < records.length; i++) {
    const record = records[i];
    let id = nextCampaignId(ids.concat(assigned));
    while (await getCampaign(id)) id = nextCampaignId(ids.concat(assigned, id));
    // The examples reference their parent by the position it was built in,
    // because nothing has an id until now.
    if (record.parentId) record.parentId = idByParentIndex[record.parentId] || record.parentId;
    record.id = id;
    idByParentIndex[record.name] = id;
    await setRaw(mmKeys.campaign(id), record);
    assigned.push(id);
  }
  await setRaw(mmKeys.index(), ids.concat(assigned));
  return { created: assigned.length, ids: assigned };
}

/** Count the examples currently loaded. */
export async function demoCount() {
  return (await listCampaigns()).filter((c) => c && c.demo === true).length;
}

/**
 * Remove every example, and only the examples: matched on the flag the loader
 * wrote, never on the name, because a real campaign somebody calls "Example
 * mailer" is not ours to delete.
 */
export async function removeDemo() {
  const all = await listCampaigns();
  const demo = all.filter((c) => c && c.demo === true);
  const ids = await readIndex();
  const gone = new Set(demo.map((c) => String(c.id)));
  await Promise.all(demo.map((c) => setRaw(mmKeys.campaign(c.id), null)));
  await setRaw(mmKeys.index(), ids.filter((id) => !gone.has(String(id))));
