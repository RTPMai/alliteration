...
export const CAMPAIGN_TYPES = [
  ...
  },
  {
    ...
    steps: [
      ...
      step("poll_copy", "build", "Write the poll email: 3 to 5 focused questions", AM, null,
        { help: "Test for clarity, bias, burden and too many open-ended questions. Never promise to share the findings." }),
  ...
  },
  {
    ...
    steps: [
      ...
      step("picks_select", "build", "Pick at least five favorites, each one complete", AM, null,
        { help: "Each pick: main image, product name, style number, MSRP, personal reason, color swatches, total colors, and direct vendor link. Fewer than five COMPLETE picks cannot be approved." }),
  ...
  /* ---- relationship and fulfillment ---- */
  {
    ...
    steps: [
      ...
      step("ref_revenue", "track", "Track first orders and revenue through November 30", AM, { fixed: "11-30" },
        { help: "From the first order through November 30 of that fiscal year. A December first order runs to the FOLLOWING November 30, never a rolling 12 months." }),
      step("ref_rank", "track", "Rank confirmed referrals and name P&M's Premier Promoter", "Jacob and " + AM, { fixed: "11-30" },
        { help: "Ranked on distinct confirmed referred people, not entries or revenue. Revenue is reported separately. Rankings stay internal." }),
      step("ref_tie", "track", "If tied: start the Promoter Prize-fight and its connected Digital Platform campaign", "Jacob", null,
        { na: true, help: "About one week after this year's actual Spotify Wrapped release. Revenue is never the tie-breaker." }),
    ],
  },
  {
    ...
    steps: [
      ...
      step("samp_followup", "track", "Contact recipients 5 to 7 days after confirmed delivery", AM, { days: 9 },
        { after: ["samp_delivered"], help: "Counted from confirmed delivery, never from the ship date. Follow up within one business day of any response." }),
      postLaunchReview(),
  ...
  /* ---- gifting and community ---- */
  {
    ...
    steps: [
      step("iog_theme", "create", "Choose the theme and the client need-in-hand date together", "Jacob, Ryan, and Megan", null,
        { help: "Distribution is two months before clients need the promoted items. Not the older one-month timing." }),
  ...
  },
  {
    ...
    steps: [
      step("par_choose", "create", "Choose the parade and theme together", "Margo, Ryan, and Megan", null,
        { help: "Ankeny is an option, not a mandatory event. Confirm this year's organizer rules rather than assuming last year's." }),
      step("par_plan", "plan", "Set the giveaway quantity and check the stockroom first", "Margo and Production", null,
        { help: "About 2,000 shirts is a past example, not a fixed requirement. Record usable stock and order only the shortfall." }),
  ...
  {
    key: "try_on_day", label: "Try On Day", family: "live",
    controlLabel: "Event date",
    summary: "Take garment samples to a client so their people can try things on and order from their online store.",
    steps: [
      step("tod_qualify", "create", "Qualify the client and the group", AM, { days: -56 },
        { help: "Needs an active or scheduled online store, an indoor location, a client contact who will promote it, a clear ordering window, and at least 25 participants. 25 is a firm minimum; anything less is Jacob's exception to approve." }),
      step("tod_format", "create", "Choose the format and confirm the arrangements", AM, null,
        { help: "P&M staffed on site, client-hosted rack loan, shipped rack or stand, or an approved other. Explain upfront purchase, the stockroom return, care and replacement, and delivery costs before anyone commits." }),
      step("tod_calendar", "plan", "Open the campaign and enter every date", AM, null,
        { help: "Working, Art, review, event, return, weekly results and year-end dates. Match existing BackBone records rather than creating duplicate clients." }),
      step("tod_samples", "plan", "Choose samples and explain the purchase policy", AM, { days: -28 },
        { help: "Standard is one Medium and one 2XL per style unless another mix is approved. EVERY sample is bought upfront, including undecorated ones, and all of them come back. Never promise attendees can keep a sample." }),
      step("tod_invoice", "build", "Create the Printavo invoice and paste the link", AM, { days: -28 },
        { help: "Named [client] + Try On Day under Sample Ordering. Include the event details, garments, decoration, format and logistics." }),
      step("tod_jacob", "build", "Jacob reviews the invoice", "Jacob", null,
        { approval: true, after: ["tod_invoice"], help: "The second review happens before anything is purchased or produced." }),
      step("tod_payment", "build", "Collect payment and set the return date", AM, null,
        { after: ["tod_jacob"], help: "Purchasing and production wait for payment and Jacob's review. The return date is its own date, normally one week after the event, and is never Printavo's invoice date." }),
      step("tod_store", "build", "Check the store, the QR code and the ordering dates", AM, { days: -21 },
        { help: "Check every product, name, color, price and size against the store. Test the QR on a phone. An anonymous scan is not an identified buyer." }),
      step("tod_kit", "build", "Pull, inspect and count the samples; prepare the kit", "Purchasing and Shipping", { days: -14 },
        { after: ["tod_payment"], help: "Link the outgoing lines to the stockroom source and the Printavo invoice. The same lines are counted back on return." }),
      step("tod_announce", "build", "Client distributes the announcement", AM, { days: -14 },
        { help: "The Account Manager writes it, the client sends it. Record the distribution date and evidence separately." }),
      prelaunchReview({ days: -7 }, { label: "Prelaunch review: kit, staffing, custody, logistics, go or no-go" }),
      step("tod_reminder", "launch", "Client distributes the reminder", AM, { days: -2 },
        { ...AFTER_REVIEW, help: "One to three business days before. Time, location, store link and deadline." }),
      step("tod_event", "launch", "Run the event: handoff, setup, fit support, ordering help", "Assigned staff or client host", LAUNCH_DAY, AFTER_REVIEW),
      step("tod_thanks", "track", "Thank the client and send the post-event ordering email", AM, { bd: 1 },
        { after: ["tod_event"], help: "Store link, deadline, instructions and the return plan." }),
      step("tod_return", "track", "Samples and equipment come back", "Purchasing and Shipping", { days: 7 },
        { after: ["tod_event"], help: "Overdue returns stay flagged until receipt is documented." }),
      step("tod_count", "track", "Inspect and count usable garments back into stockroom inventory", "Purchasing and Shipping", { days: 7 },
        { after: ["tod_return"], help: "Outgoing equals usable returned plus damaged returned plus missing. Post the verified usable count once; saving again must never add stock twice. The return is an inventory step, not a credit or refund." }),
      step("tod_first_results", "track", "First results and repeat-value review", AM, { days: 14 },
        { help: "Orders, sales, feedback and open actions." }),
      postLaunchReview(AM, { days: 14 }),
      step("tod_tracking", "track", "Weekly results through three months, then the year-end review", AM, { days: 90 },
        { help: "Tracking continues after the store closes. Returning the samples does not end it." }),
    ],
  },
  {
