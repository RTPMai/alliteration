...
export function scorePair(a, b) {
  ...
  if (score < 62 && shareRatio >= 0.6 && sharedCount >= 2) {
...
export function suggestDuplicates(synced, opts) {
  ...
  const addTo = (key, row) => {
    ...
    // A block with hundreds of rows in it is a word like "school", not a
    // signal. Capped so one common token cannot make this quadratic again.
    if (bucket.length < 40) bucket.push(row);
  };
  ...
  live.forEach((row) => {
    ...
    emailsOf(row).forEach((e) => {
      ...
      if (d && !FREE_MAIL.has(d)) addTo("d:" + d, row);
    });
  });
