// PUT IN: lib/mailme/resend-client.js
...
/**
 * WHAT HAPPENED TO ONE SENT EMAIL, asked on demand rather than pushed.
 *
 * Resend keeps a record per message and reports the latest thing that
 * happened to it. MailMe learns the same facts through a webhook, which is
 * the right shape there: it is aggregating thousands of sends into counts
 * nobody opens one at a time. PromoPro asks about ONE purchase order, at the
 * moment somebody has that order on screen, so a pull costs one call exactly
 * when the answer is wanted and needs no route, no secret and no cron.
 *
 * Returns { ok, status, at, error }. `status` is Resend's own last-event
 * string, lowercased, or "" when it has nothing. NEVER THROWS: an order whose
 * delivery status cannot be read is still an order, and losing the whole
 * screen because this call failed would be a worse outcome than not knowing.
 */
export async function getEmailStatus(id) {
  const emailId = String(id || "").trim();
  if (!emailId) return { ok: false, status: "", at: "", error: "no message id was recorded for this send" };
  if (!resendConfigured()) return { ok: false, status: "", at: "", error: "RESEND_API_KEY is not set" };

  try {
    const body = await resendFetch(`/emails/${encodeURIComponent(emailId)}`, { method: "GET" });
    // Bare object and { data: {...} } have both turned up in Resend's own
    // examples, same as the inbound reader above, so read either.
    const d = (body && body.data && typeof body.data === "object") ? body.data : body;
    const raw = (d && (d.last_event || d.lastEvent || d.status)) || "";
    return {
      ok: true,
      status: String(raw).toLowerCase().replace(/^email\./, ""),
      at: String((d && (d.created_at || d.createdAt)) || ""),
      error: "",
    };
  } catch (e) {
    return { ok: false, status: "", at: "", error: (e && e.message) || String(e) };
  }
}
