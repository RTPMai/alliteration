...
export async function publicListSignup({ email, name, tag, listName }) {
  ...
  if (existing) {
    ...
    const already = tags.some((t) => String(t).trim().toLowerCase() === cleanTag.toLowerCase());
    if (!already) {
      tags.push(cleanTag);
      await setContactStatus(existing.id, { tags }, null);
    }
    contactId = existing.id;
  } else {
    const { added } = await addProspects(
      [{ email: cleanEmail, contact_name: cleanName, tags: [cleanTag] }],
      null,
      null,
    );
