...
async function getGivingRequests() {
  const raw = await kvGet("givinggauge_requests");
  if (!raw) return [];
  const data = unwrap(raw);
  if (Array.isArray(data)) return data;
...
function leadContact(lead) {
  const direct = lead.contact_email || lead.email;
  if (direct && String(direct).trim()) {
    return { email: String(direct).trim(), name: lead.contact_name || "", title: lead.contact_title || "" };
  }
  ...
  const withEmail = contacts.find((c) => c && c.email && String(c.email).trim());
  if (!withEmail) return null;
  return {
    email: String(withEmail.email).trim(),
    name: withEmail.name || "",
    title: withEmail.title || "",
  };
}
...
export async function setContactStatus(contactId, patch, session) {
  ...
  if (patch.tags !== undefined) {
    if (source === "prospect") {
      ...
      // their tags in its own overrides map under the FULL prefixed id
      ...
      if (!patch.tags.length && !all[id].status && !all[id].reason) delete all[id];
