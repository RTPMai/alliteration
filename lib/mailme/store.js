...
import { getReorderSettings } from "../reorder-settings.js";
...
// Enough to cover any realistic campaign's worth of in-flight events
// several times over, while keeping one KV value from growing forever.
// Oldest entries fall off first; a months-old open is not worth the space.
const MESSAGE_INDEX_MAX = 20000;

/**
 * Remember which campaign and contact each provider message id belongs to.
 *
 * Tags are the primary attribution path, but they depend on the provider
 * echoing them back in the shape we expect, and a tag that does not come
 * back leaves an event that cannot be attributed to anyone. The message id
 * always comes back, so this is the path that does not rely on anyone
 * else's behaviour.
 *
 * Best-effort: failing to write the index must never fail a send that has
 * already gone out.
 */
export async function rememberSentMessages(entries) {
  const list = (Array.isArray(entries) ? entries : []).filter((e) => e && e.id);
  if (!list.length) return;
  try {
    const all = await readObject(keys.messageIndex());
    list.forEach((e) => {
      all[String(e.id)] = { c: e.campaignId || null, k: e.contactId || null };
    });
    const ids = Object.keys(all);
    if (ids.length > MESSAGE_INDEX_MAX) {
      // Object key order is insertion order for string keys, so the oldest
      // are at the front.
      const trimmed = {};
      ids.slice(ids.length - MESSAGE_INDEX_MAX).forEach((k) => { trimmed[k] = all[k]; });
      await writeKey(keys.messageIndex(), trimmed);
      return;
    }
    await writeKey(keys.messageIndex(), all);
  } catch (e) {
    console.error("[mailme] could not record message index:", e && e.message);
  }
}

/** { campaignId, contactId } for a provider message id, or null. */
export async function lookupSentMessage(messageId) {
  if (!messageId) return null;
  try {
    const all = await readObject(keys.messageIndex());
    const hit = all[String(messageId)];
    return hit ? { campaignId: hit.c, contactId: hit.k } : null;
  } catch (e) {
    return null;
  }
}
...
export async function resolveContacts(opts) {
  ...
  // Reorder timing is BackBone's, not MailMe's: it describes the customer
  // relationship, not the email. MailMe reads it. settings.reorder is passed
  // as the legacy fallback so an install that tuned these numbers back when
  // they lived in MailMe's blob keeps them until BackBone saves its own.
  const reorderCfg = await getReorderSettings(settings.reorder);
  ...
  for (const c of synced) {
    ...
    contacts.push(decorate({
      ...
      reorder: reorderStatus(c, { ...reorderCfg, now }),
...
/**
 * Delete a campaign. A draft only, unless `opts.allowSent` is passed, which
 * the route does ONLY after deleteDecision() in lib/mailme/access.js has
 * confirmed an Admin is asking. A sent campaign's click/open events go with
 * it, so its report disappears too. Suppression (unsubscribes, bounces,
 * complaints) lives on the contact and is never touched here.
 */
export async function deleteCampaign(id, opts) {
  const allowSent = !!(opts && opts.allowSent);
  const all = await readArray(keys.campaigns());
  const idx = all.findIndex((c) => c.id === id);
  if (idx === -1) return { ok: false, reason: "not_found" };
  const status = all[idx].status;
  if (status === "sending") return { ok: false, reason: "locked" };
  if (status !== "draft" && !allowSent) return { ok: false, reason: "locked" };
  all.splice(idx, 1);
  await writeKey(keys.campaigns(), all);
  if (status !== "draft") await kvPipeline([["DEL", keys.campaignEvents(id)]]);
  return { ok: true };
}
