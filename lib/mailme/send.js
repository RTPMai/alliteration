...
function renderInline(escapedText) {
  ...
  out = out.replace(/(^|[\s(])((?:https?:\/\/|www\.)[^\s<>"']+)/gi, (m, before, url) => {
    ...
    return `${before}<a href="${escapeAttr(href)}" style="color:#3a6fb0">${url}</a>${tail}`;
  });
...
export async function sendCampaign(id, session, opts) {
  ...
  if (!String(campaign.subject || "").trim()) missingContent.push("This email has no subject line.");
  if (!String(campaign.body || "").trim()) missingContent.push("This email has no body.");
