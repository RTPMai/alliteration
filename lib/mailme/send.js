...
export function templateContext(campaign, contact, settings, unsubToken, opts) {
  ...
  return {
    ...
    // Who this reader's account manager is in BackBone ("Hannah Posey").
    // The seasonal design turns it into their own "Email Hannah" button.
    accountManager: String((contact && contact.accountManager) || ""),
 ...
 */
export async function sendTestEmail(campaignId, toEmail, opts) {
  ...
  //
  // The composer can ask for the copy a given account manager's clients get
  // (opts.accountManager), so a test of the seasonal design shows the
  // "Email Hannah" button, and its reply-to, exactly as Hannah's clients
  // will see them. Left blank it is the no-account-manager copy.
  const asAm = String((opts && opts.accountManager) || "").trim().slice(0, 60);
  const previewContact = { id: `test:${to}`, email: to, contact_name: "", company_name: "Test Company", accountManager: asAm };
  const token = makeToken(previewContact.id);

  // With no account manager asked for, this resolves to the fixed address
  // when one is set and is omitted otherwise. That mirrors what a real
  // recipient with no AM on file would get.
