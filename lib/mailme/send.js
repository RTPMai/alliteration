<!-- PUT IN: lib/mailme/send.js (rename this file, drop the .txt) -->
...
function renderInline(escapedText) {
  ...
  out = out.replace(/\[([^\]\n]+)\]\((https?:\/\/[^\s)]+)\)/g,
    (m, text, url) => `<a href="${escapeAttr(url)}" style="color:#3a6fb0">${text}</a>`);
  return out;
}
...
export async function sendTestEmail(campaignId, toEmail) {
  ...
  const from = (settings.fromAddress && settings.fromAddress[identity.key]) || "";
  if (!from.trim()) {
    blockers.push({
      field: `fromAddress.${identity.key}`,
