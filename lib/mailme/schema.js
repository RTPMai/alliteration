...
import { TEMPLATES, TEMPLATE_KEYS, FREEFORM, MAX_TEMPLATE_DATA_BYTES } from "./templates/index.js";
...
export const keys = {
  ...
  // provider message id -> which campaign and contact it was. See
  // rememberSentMessages(): attribution that does not depend on tags
  // surviving the provider's round trip.
  messageIndex: () => `${KEY_PREFIX}:msgindex`,
...
export const CAMPAIGN_FIELDS = {
  ...
  body:        { type: "string", required: true,  source: "user",      def: "Body. Supports {{first_name}} / {{company_name}} merge tags. Unused when template is not freeform." },
  template:    { type: "enum",   required: false, enum: TEMPLATE_KEYS, source: "user", def: "freeform (the body text box) or a designed template from lib/mailme/templates/." },
  templateData:{ type: "object", required: false, source: "user",      def: "A template's content (products, picks, copy). Shape owned by the template module." },
...
export function newCampaignDraft(partial, session) {
  ...
  return {
    ...
    // THE LINK TO MARKETMACHINE, and the only place it is stored. A marketing
    // campaign finds its emails by looking for these; it does not keep its own
    // list of email ids, because two copies of one fact drift the moment an
    // email is deleted here. Null means a standalone send, which stays a
    // legitimate thing to do: a one-off note to a list is not a campaign.
 ...
 */
/**
 * Which product and which spot on the card a click came from, read back out
 * of the utm_content tag the designed templates put on every link (tagLink
 * in lib/mailme/templates/shared.js). Null for a link without one, which is
 * every freeform link.
 *
 *   "pick-03-photo" -> { key, slot: 3, spot: "photo", label: "Pick 03 · photo" }
 *   "hero"          -> { key, slot: 0, spot: "hero",  label: "Top button" }
 */
const SPOT_LABELS = { photo: "photo", button: "button", colors: "colors", text: "plain-text link" };
const GENERAL_SPOTS = { hero: "Top button", closing: "Bottom button", footer: "Footer link", form: "Inquiry form button", calendar: "Calendar picture" };
export function clickSpotFromUrl(url) {
  let content = null;
  try { content = new URL(String(url || "")).searchParams.get("utm_content"); } catch (e) { return null; }
  if (!content) return null;
  // Only the spots the templates write. Anything else in utm_content
  // is somebody else's tag (or a hand-typed one) and is not a spot; letting
  // an arbitrary word through would let "pick-01-unique" overwrite the
  // counters below (Sep 24 2026 review).
  const m = /^pick-(\d{2})-(photo|button|colors|text)$/.exec(content);
  if (m) {
    const slot = parseInt(m[1], 10);
    return { key: content, slot, spot: m[2], label: `Pick ${m[1]} \u00b7 ${SPOT_LABELS[m[2]] || m[2]}` };
  }
  if (GENERAL_SPOTS[content]) return { key: content, slot: 0, spot: content, label: GENERAL_SPOTS[content] };
  return null;
}

/**
 * Clicks per product for a template email: how many clicks each pick got
 * and from how many people, plus which spot on the card got them. The
 * sales director's questions, answered from the same click events as
 * everything else: which pick won, and do people click the photo, the
 * colors, or the button.
 */
function summarizeSpots(clickEvents) {
  const picks = new Map();
  const spots = new Map();
  for (const e of clickEvents) {
    const where = clickSpotFromUrl(e.linkUrl);
    if (!where) continue;
    const who = e.contactId ? String(e.contactId) : null;
    if (where.slot > 0) {
      if (!picks.has(where.slot)) picks.set(where.slot, { slot: where.slot, clicks: 0, unique: new Set(), photo: 0, button: 0, colors: 0, text: 0 });
      const row = picks.get(where.slot);
      row.clicks++;
      if (who) row.unique.add(who);
      if (row[where.spot] !== undefined) row[where.spot]++;
    }
    const spotKey = where.slot > 0 ? where.spot : where.key;
    const spotLabel = where.slot > 0 ? (SPOT_LABELS[where.spot] || where.spot) : where.label;
    if (!spots.has(spotKey)) spots.set(spotKey, { spot: spotKey, label: spotLabel, clicks: 0 });
    spots.get(spotKey).clicks++;
  }
  return {
    byPick: [...picks.values()].sort((a, b) => a.slot - b.slot)
      .map((r) => ({ slot: r.slot, clicks: r.clicks, uniqueClicks: r.unique.size, photo: r.photo, button: r.button, colors: r.colors, text: r.text })),
    bySpot: [...spots.values()].sort((a, b) => b.clicks - a.clicks),
  };
}
...
export function aggregateEvents(events, recipientCount) {
  ...
  const links = [...byLink.values()]
    .map((r) => {
      const where = clickSpotFromUrl(r.url);
      return { url: r.url, clicks: r.clicks, uniqueClicks: r.unique.size, ...(where ? { spot: where.label } : {}) };
    })
    .sort((a, b) => b.clicks - a.clicks);

  const { byPick, bySpot } = summarizeSpots(list.filter((e) => e && e.type === "click"));
  return { stats, links, byPick, bySpot };
}
...
export function validateCampaignPatch(body) {
  ...
  // DESIGNED TEMPLATES. The content is cleaned by the template's own
  // normalize(), never stored as the browser sent it, and it is capped in
  // size. A template draft may be incomplete: what is missing is reported by
  // the template's problems() and refused at SEND time (sendCampaign), the
  // same rule the subject and body follow.
  if (b.template !== undefined) {
    const k = String(b.template || FREEFORM);
    if (!TEMPLATE_KEYS.includes(k)) errors.push(`template must be one of: ${TEMPLATE_KEYS.join(", ")}`);
    else patch.template = k;
  }
  if (b.templateData !== undefined) {
    const key = patch.template;
    if (b.templateData === null) {
      patch.templateData = null;
    } else if (!key) {
      errors.push("templateData must be sent together with template");
    } else if (key === FREEFORM) {
      patch.templateData = null;
    } else if (typeof b.templateData !== "object" || Array.isArray(b.templateData)) {
      errors.push("templateData must be an object");
    } else {
      const clean = TEMPLATES[key].normalize(b.templateData);
      if (JSON.stringify(clean).length > MAX_TEMPLATE_DATA_BYTES) errors.push("This email has too much content to save.");
      else patch.templateData = clean;
    }
  }
  ...
  if (b.marketingCampaignId !== undefined) {
    patch.marketingCampaignId = b.marketingCampaignId ? String(b.marketingCampaignId) : null;
  }
  if (b.marketingChannelId !== undefined) {
    patch.marketingChannelId = b.marketingChannelId ? String(b.marketingChannelId) : null;
  }
  // Detaching from a campaign must clear BOTH halves. A channel id left
  // pointing at a campaign the email no longer belongs to would make the
  // rollup count reach against a channel item that is not linked to anything.
  if (patch.marketingCampaignId === null) patch.marketingChannelId = null;
...
export function defaultSettings() {
  return {
    ...
    policy: {
      ...
      coldDailyCapMax: 200,
      coldRampDays: 30,
      // Resend's free tier allows 100 emails a day across the whole account.
      // A cap above that does not send more, it starts a send that fails part
      // way through. Raise this to match the plan the day the account is
      // upgraded, not before. (Sep 23 2026: free tier confirmed.)
      clientDailyCap: 100,
      skipOpenQuotes: true,
