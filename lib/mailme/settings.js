...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "PATCH") {
      ...
      // REORDER TIMING MOVED. It lives in BackBone now (lib/reorder-settings.js,
      // PATCH /api/reorder-settings), because it describes the customer
      // relationship rather than the email. A `reorder` key arriving here is
      // ignored rather than rejected: an old browser tab still holding the
      // previous Settings screen should not have its whole save fail over a
      // field that simply moved. The stored blob keeps its old value as the
      // legacy fallback the shared store reads on a fresh install.
