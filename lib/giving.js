...
import { readRoster } from "./backbone-store.js";
...
export function buildRequest(submission, meta = {}) {
  ...
  return {
    ...
    // Filled in later by matching the BackBone roster. `found:false` scores relationship and
 ...
 *
 * Not everything arrives on the Jotform. A school calls, somebody asks at the
 * counter, or a donation that went out two years ago needs to be on the books
 * so the giving totals are not a partial picture. Those all have to land in
 * the same queue, scored by the same engine, or GivingGauge only ever answers
 * for the requests that happened to use the form.
 *
 * IT GOES THROUGH buildRequest LIKE EVERYTHING ELSE. The typed fields are
 * turned into a form-shaped submission first, so there is ONE mapping, one
 * piece-count parser and one address parser rather than a second set that
 * drifts. It also means `raw` holds something repairRequest() can re-read
 * later, exactly like an imported row.
 *
 * The only thing marked differently is `source`, which is "manual". That is
 * what tells the screen, and anybody reading the record later, that no form
 * was filled in and these answers came from a person.
 */
export function buildManualRequest(input, meta = {}) {
  const f = input && typeof input === "object" ? input : {};
  const val = (v) => (v == null ? "" : String(v).trim());

  const orgName = val(f.orgName);
  if (!orgName) throw new Error("Organization name is required");

  // Keys are the exact labels FIELD_ALIASES already knows. Anything blank is
  // left out rather than sent as an empty string, so an absent answer reads as
  // absent and lands on the review list instead of looking answered.
  const answers = {};
  const put = (label, v) => { const s2 = val(v); if (s2) answers[label] = s2; };

  put("Organization Name", orgName);
  put("Contact Name", f.contactName);
  put("Email", f.email);
  put("Phone Number", f.phone);
  put("Event Name", f.eventName);
  put("Type of Event", f.eventType);
  put("Date of Event", f.eventDate);
  put("City", f.city);
  put("State", f.state);
  put("Address", f.address);
  put("Estimated Event Attendance", f.attendance);
  put("Years Event Has Been Active", f.yearsActive);
  put("Description of Event", f.description);
  put("Merchandise Type Requested", f.merchandise);
  put("How many pieces are you requesting?", f.pieceCount);
  put("How will merchandise be used?", f.merchUse);
  put("Will you be purchasing any apparel for this event?", f.purchaseIntent);
  put("Describe why you feel P&M Apparel should honor your request", f.rationale);
  put("Additional information about your request", f.additional);
  put("Tax Status", f.taxStatus);
  put("EIN / Tax ID Number", f.ein);

  const submission = { answers: {} };
  Object.keys(answers).forEach((label, i) => {
    submission.answers[String(i + 1)] = { text: label, answer: answers[label] };
  });

  const row = buildRequest(submission, {
    id: meta.id,
    submittedAt: meta.submittedAt || new Date().toISOString(),
    source: "manual",
    jotformId: null
  ...
  row.enteredBy = val(meta.enteredBy);

  // A classification typed in on the way is applied straight away, because
  // whoever is entering a request by hand already knows what kind of
  // organisation it is. Left alone when it is not supplied.
  const ORG_TYPES = ["nonprofit", "school", "youth", "civic", "religious",
                     "political", "business", "individual"];
  const MISSION_FITS = ["core", "adjacent", "civic", "promotional", "contrary"];

  if (ORG_TYPES.includes(val(f.orgType))) row.request.orgType = val(f.orgType);
  if (MISSION_FITS.includes(val(f.missionFit))) {
    row.request.missionFit = val(f.missionFit);
    row.needsReview = row.needsReview.filter((n) => n.field !== "classification");
  }
  if (f.isReligious === true || f.isReligious === false) row.request.isReligious = f.isReligious;
  if (f.isPolitical === true || f.isPolitical === false) row.request.isPolitical = f.isPolitical;
...
export async function updateRequest(id, patch) {
  ...
  // What the donation actually cost, recorded after approval.
  //
  // Two figures, because they answer different questions. Retail value is what
  // the goods would have sold for and is the number an org wants on a receipt;
  // cost is what P&M actually gave up, and is the honest one to weigh against
  // what a customer spends with us. Either can be left blank.
  if (patch.fulfillment) {
    const f = row.fulfillment || {};
    const p = patch.fulfillment;
    const num = (v) => {
    ...
    if (p.retailValue !== undefined) f.retailValue = num(p.retailValue);
    if (p.cost !== undefined) f.cost = num(p.cost);
    if (p.notes !== undefined) f.notes = String(p.notes || "");
    if (p.fulfilledAt !== undefined) f.fulfilledAt = p.fulfilledAt || null;
    if (p.recordedBy !== undefined) f.recordedBy = p.recordedBy || "";

    // Default the date to the day it was recorded, so month and year rollups
    // always have something to bucket by.
    if (!f.fulfilledAt) f.fulfilledAt = new Date().toISOString().slice(0, 10);

    row.fulfillment = f;
  }

  // Reopening a request clears the decision but never the recorded spend: if
  // product went out the door, that stays on the books.
