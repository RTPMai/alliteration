// PUT IN: lib/websitewidget/store.js (REPLACES the current one)
...
function cacheKey(siteId, days) {
  return `${PREFIX}stats:${siteId}:${days}`;
}

export async function getCached(siteId, days) {
  const cfg = config();
  if (!cfg) return null; // fail open: no KV configured just means no cache, not an error

  try {
    const res = await fetch(`${cfg.url}/get/${encodeURIComponent(cacheKey(siteId, days))}`, {
...
export async function setCached(siteId, days, stats) {
  const cfg = config();
  if (!cfg) return;

  const payload = { ...stats, cachedAt: new Date().toISOString() };
  try {
    await fetch(`${cfg.url}/set/${encodeURIComponent(cacheKey(siteId, days))}`, {
      method: "POST",
