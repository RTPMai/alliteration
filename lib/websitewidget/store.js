...
// Exported so a test can call it for real rather than grepping this file
// for the shape of the key. The comparison mode is part of the key: a
// "vs last year" pull and a "vs previous period" pull over the same site
// and range are different payloads, and keying on site and range alone
// would serve one under the other for ten minutes.
export function cacheKey(siteId, days, compare = "none") {
  return `${PREFIX}stats:${siteId}:${days}:${compare || "none"}`;
}

export async function getCached(siteId, days, compare = "none") {
  const cfg = config();
  if (!cfg) return null; // fail open: no KV configured just means no cache, not an error

  try {
    const res = await fetch(`${cfg.url}/get/${encodeURIComponent(cacheKey(siteId, days, compare))}`, {
...
export async function setCached(siteId, days, stats, compare = "none") {
  const cfg = config();
  if (!cfg) return;

  const payload = { ...stats, cachedAt: new Date().toISOString() };
  try {
    await fetch(`${cfg.url}/set/${encodeURIComponent(cacheKey(siteId, days, compare))}`, {
      method: "POST",
