...
async function fetchWindow(propertyId, window, keyFilters = null) {
  ...
  return {
    ...
    trend: rowsToObjects(trendReport).map((r) => ({
  ...
  try {
    ...
    if (text.includes("(403)") || text.includes("denied access")) {
      return {
        ...
        serviceAccount: process.env.GA4_CLIENT_EMAIL || null,
        message:
          `Property ${id} exists but has not given this service account access. In ` +
          `analytics.google.com, open that property's Admin, then Property Access ` +
          `Management, and add ${process.env.GA4_CLIENT_EMAIL || "the service account"} as a Viewer.`
      };
    }
