...
export function requireAuth(req, res, requiredRole) {
  ...
  // ROLES ARE GONE, Sep 2026. A role name in a cookie is a claim issued at
  // sign-in; the Admin flag is storage. Any route that used to pass "admin"
  // here now reads permsFor() itself, which is both live and the only
  // definition. This argument is kept so nothing breaks on deploy, and it is
  // deliberately inert: silently allowing is wrong, silently refusing every
  // caller is worse, so a route that still passes one gets told.
  if (requiredRole) {
    console.warn("requireAuth: role argument is ignored, roles were removed. Check permsFor() instead.");
  }
  return sess;
}
