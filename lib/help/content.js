...
edit rights. Deleting is administrators only.`,
  ...
  {
    app: "crewcore",
    title: "CrewCore (employees, time clock, time off, stipend)",
    keywords: ["employee", "hr", "staff", "team", "roster", "clock",
      "punch", "timecard", "hours", "overtime", "stipend", "apparel",
      "review", "one on one", "handbook", "pay", "rate", "pto", "time off",
      "vacation", "day off", "request off", "approve",
...
TIME OFF (added Sep 2026). CrewCore is where PTO lives now: requests,
approvals and balances. Everybody with CrewCore has a Time Off tab. Pick a
first and last day, the hours fill in from weekdays times the work day (change
them for a half day), add a note if you like, and send it. Only the people
ticked as approvers in CrewCore Settings can approve or deny, and the person
who asked gets a notification either way. A request can be cancelled by the
person who made it until it is answered; after that, ask an approver.

Balances: hours are granted every Jan 1 by how many full years somebody has
worked (set in Settings), and a new hire's first year is prorated from their
start date. Unused hours carry into the next year up to the cap in Settings.
Pending requests are not taken off the balance until approved. Approvers can
log time off for somebody (a sick call) or adjust a balance with a reason
(event comp time). Somebody marked PTO exempt on the Roster (the owners) has
no balance; time off they log is approved on the spot and still shows who is
out.

LOGINS AND EMAILS ON THE ROSTER. The Roster flags anyone with no login (they
cannot sign in) or no email (they will not get time off emails). Adding
somebody to the Roster makes them a login unless you untick it, and an
existing person with no login has a "Make a login" button on their record.
The temporary password is shown once; if it is lost, reset it in Settings
under Accounts. New logins get CrewCore only.`,
  },
...
a row stops retrying and waits for a person.`,
...

  {
    ...
    body: `WHO CAN SEE IT. Tick MarketMachine on somebody's account in Settings and they
get My tasks, and only My tasks. To give somebody more, tick the screens you
want them to have on that same account: ticking Campaigns gives the whole app,
because a campaign page carries its budget, numbers, connections and history
with it. That is how the sales director has everything in MarketMachine without
the platform Admin flag, which would also hand him pay, reviews and accounts.
The rail and the server read the same rule, so neither half can be worked
around. People see a change the next time they sign in.

MY TASKS is the first screen and the simple way in. It lists only the open
...
data.`,
...

  {
    ...
    keywords: ["embroidery", "stitch", "stitches", "count", "dst", "digitize",
      "quote", "pricing", "artwork", "colorway", "thread", "guess", "recolor",
      "ink", "mockup"],
...
DST or the customer's artwork (PNG, JPG, PDF, AI), lets you swap colours
against a garment colour, and exports a picture. Stitch
...
COLORWAY WITH ARTWORK. Drop a PNG, JPG, PDF or AI and the colours in the art
are found and listed, biggest first. Change a swatch and everything in that
colour changes, soft edges included. Set two swatches to the same colour to
merge them. "Colours to find" raises or lowers how many it looks for. A white
box behind a logo is removed automatically, but only where it touches the
edge, so white lettering inside the design stays. The "colours in use" count
under the preview is the screen count for print or the thread count for
embroidery. It flattens photos and gradients into flat colours, so it suits
logos, not photographs.`,
  },
...
EIGHT SCREENS. Home is what is committed, collected, outstanding, and waiting
...
Social is the posting plan, week by week.

THE SOCIAL SCREEN. The plan comes in as a file (the .json the plan was written
as) and an admin loads it with the Load plan file button. Every post shows
under its week with its channels, what it points people to, and whether it has
copy yet. Click a post to edit the copy or the date, or to mark it posted or
skipped. Mark posted is also a button right on the row. Some posts only run if
something happens first: a decision (price, registration date) or a real-world
event (a Gold sponsor signs, a speaker confirms). Decisions are answered in the
list at the top of the screen; events are ticked inside the post. Open
decisions, late posts, and posts going out within a week with no copy or still
waiting on something all show on Home. Loading a newer copy of the plan file
adds new posts and updates the rest, but never undoes anything a person did:
posted stays posted, rewritten copy stays rewritten, a made decision stays
made. A post dropped from the new file is left alone rather than deleted.
