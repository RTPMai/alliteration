...
export function rosterGaps(employee, logins) {
  ...
  // Either address counts (Sep 23 2026): somebody with only a personal
  // email still gets their time off emails, which is what this flag is for.
  const ok = (x) => /^[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+$/.test(String(x || "").trim());
  const u = String(e.username || "").trim().toLowerCase();
  return {
    no_email: !ok(e.email) && !ok(e.personal_email),
