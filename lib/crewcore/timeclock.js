...
/* ---- storage anchor vs pay week ------------------------------------------
 *
 * WHY THESE EXIST, Sep 2026.
 *
 * The week key used to be computed from the shop's configurable week start
 * and then BAKED INTO THE STORAGE KEY. So switching the setting from Sunday
 * to Monday made every bucket ever written unreachable in one click: the
 * reader asked for Monday-anchored keys that had never been written, found
 * nothing, and every employee showed 0.00 for every week going back to the
 * beginning. Nothing was deleted and there was no error. It just went quiet.
 *
 * That is a presentation choice inside a storage key, and it should never
 * have been able to orphan history.
 *
 * So storage is now anchored to Sunday, always, whatever the setting says.
 * Sunday because that is the default the existing data was written under, so
 * the overwhelming majority of buckets on disk already match and need no
 * migration.
 *
 * The pay week is applied when a screen ASKS. A Monday-start week spans two
 * Sunday buckets, so a read collects the overlapping buckets and filters by
 * the actual local dates in the window. Changing the setting now just redraws
 * the grid.
 */

/** The anchor every shift is FILED under. Not a setting. Never a setting. */
export const STORAGE_WEEK_START = 0;

/** The bucket a local date is filed in, regardless of the shop's pay week. */
export function storageKeyFor(dateStr) {
  return weekKeyFor(dateStr, STORAGE_WEEK_START);
}

/** True if a stored key was written under the storage anchor. */
export function isStorageKey(key) {
  return dayOfWeek(key) === STORAGE_WEEK_START;
}

/** Every storage bucket overlapping an inclusive local-date window. */
export function storageKeysForWindow(startDate, endDate) {
  return weekKeysInRange(startDate, endDate, STORAGE_WEEK_START);
}

/** The seven local dates of the pay week that `startDate` begins. */
export function payWeekWindow(startDate) {
...
/* ---- pay periods -----------------------------------------------------------
 *
 * Added Sep 23 2026. P&M pays twice a month: the 1st through the 15th, and
 * the 16th through the last day of the month. So a pay period is 13 to 16
 * days long and never lines up with a work week.
 *
 * OVERTIME IS STILL WEEKLY. Federal overtime is owed past 40 hours in a WORK
 * WEEK, not in a pay period. Adding up a 15-day period and calling everything
 * past 40 overtime would show nearly the whole shop in overtime every period.
 * So overtime is figured one work week at a time, using the whole week even
 * when part of it fell in the previous period, and it is reported in the
 * period the week ENDS in. A week that runs past the end of this period is
 * listed but not counted; its overtime shows up next period, once the week is
 * actually over. Each overtime hour is counted exactly once.
 */
...
export function isDateString(str) {
  ...
  return dt.getUTCFullYear() === Number(m[1]) &&
...
/** The pay period a date falls in: { start, end }, both inclusive. */
export function payPeriodFor(dateStr) {
  const [y, m, d] = String(dateStr).split("-").map(Number);
  const ym = `${y}-${pad(m)}`;
  if (d <= 15) return { start: `${ym}-01`, end: `${ym}-15` };
  return { start: `${ym}-16`, end: `${ym}-${pad(lastDayOfMonth(y, m))}` };
}

/** The pay period n periods before (negative) or after (positive) this one. */
export function shiftPayPeriod(dateStr, n) {
  let p = payPeriodFor(dateStr);
  const steps = Math.abs(Math.trunc(Number(n) || 0));
  for (let i = 0; i < steps; i += 1) {
    p = payPeriodFor(n > 0 ? addDays(p.end, 1) : addDays(p.start, -1));
  }
  return p;
}

/** Every date in an inclusive window, oldest first. */
export function datesInWindow(start, end) {
  const out = [];
  let d = start;
  let guard = 0;
  while (d <= end && guard < 400) { out.push(d); d = addDays(d, 1); guard += 1; }
  return out;
}

/**
 * The first date a period read has to reach back to: the start of the work
 * week the period opens in. Those extra days never count as hours in this
 * period, they only decide whether that first week went over 40.
 */
export function payPeriodReadStart(period, weekStartDay = 0) {
  return weekKeyFor(period.start, weekStartDay);
}

/**
 * Rolls one employee's shifts up into a pay period. `shifts` should run from
 * payPeriodReadStart() through the period end. Same shape summarizeWeek
 * returns, so the screen draws either one, plus `weeks`: how overtime was
 * worked out, one line per work week that touches the period.
 */
export function summarizePeriod(shifts, opts = {}) {
  const {
    start, end,
    weekStartDay = 0,
    overtimeAfter = 40,
  ...
  const days = {};
  datesInWindow(start, end).forEach((d) => { days[d] = 0; });
  ...
  (shifts || []).forEach((s) => {
    const lp = localParts(s.in_at, timezone);
    if (!lp || lp.date > end) return;
    const inPeriod = lp.date >= start;
    const h = shiftHours(s);
    if (h == null) {
      if (inPeriod) {
        open += 1;
    ...
    const wk = weekKeyFor(lp.date, weekStartDay);
  ...
  const weeks = weekKeysInRange(start, end, weekStartDay).map((wk) => {
    const hours = roundHours(byWeek[wk] || 0, roundMinutes);
    const counted = addDays(wk, 6) <= end;
    const ot = counted && overtimeAfter > 0 && hours > overtimeAfter
      ? round2(hours - overtimeAfter) : 0;
    return { week_key: wk, start: wk, end: addDays(wk, 6), hours, overtime: ot, counted };
  });
  const overtime = round2(weeks.reduce((sum, w) => sum + w.overtime, 0));

  return {
    period: { start, end },
    days,
    total_hours: roundHours(total, roundMinutes),
