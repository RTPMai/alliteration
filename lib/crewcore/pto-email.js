...
//
// WHERE IT GOES. The email on their roster record. No email on file means no
...
import { requestSummary } from "./pto.js";
...
export function buildDecisionEmail(o) {
  ...
  lines.push(`PTO: ${hrs(r.hours)}`);
  if (o.note) {
    lines.push("");
    lines.push(`Note from ${o.byName || "the approver"}: ${o.note}`);
  }
  const b = o.balance;
  if (b && (event === "approved" || event === "logged" || event === "cancelled")) {
    lines.push("");
...
export async function sendDecisionEmail(o, deps = {}) {
  const to = String((o.employee && o.employee.email) || "").trim();
  ...
  const msg = buildDecisionEmail({ ...o, firstName: first });
  ...
  try {
    ...
    return { sent: true, id: (res && (res.id || (res.data && res.data.id))) || null, to };
  } catch (e) {
