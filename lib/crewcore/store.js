...
// this scale.
//
// v2, Aug 2026: PTO storage functions removed. Time off came back Sep 2026
// with its own storage in lib/crewcore/pto-store.js. Stipend spend log
...
import { keys, nextId, defaultSettings, defaultStipendFor, spendsFor, splitEmails } from "./schema.js";
...
export async function getEmployee(id) {
  // Every read splits an old one-address record into work and personal, so
  // nothing downstream ever sees a personal address in the work field.
  return splitEmails(await getRaw(keys.employee(id)));
}
...
/**
 * Total spent by one person in one calendar year. The scoping itself lives in
 * schema.spendsFor() so the store, the route and the screen all agree on what
 * counts as "in 2026".
 */
export async function usedStipendThisYear(employeeId, year) {
  const all = await listStipendSpends();
  return spendsFor(all, employeeId, year)
