...
export async function walkTriggerOrders(opts) {
  ...
  let pages = 0;
  let found = 0;
  // The deadline is wall-clock time. `now` can be injected (tests pin it to a
  // fixed date) and must not decide how long this call is allowed to run.
  const deadline = o.deadline || (Date.now() + 45000);
