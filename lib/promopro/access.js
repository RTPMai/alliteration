...
import { getUser, getAccess } from "../users.js";
...
export async function callerFor(sess) {
  ...
  const role = await getAccess(s.username);
  return { user, role };
}
 ...
 *   3. If Settings names PEOPLE, the caller must be one of them. If it names
 *      none, fall back to the shell's own can_edit, which is what this app
 *      did before the list existed. An empty list must never mean "nobody":
 *      that is what a fresh deploy looks like and it would lock the whole
 *      team out of an app that worked yesterday.
 *
 * ROLES ARE GONE, Sep 2026. This list used to name roles. It now names
 * usernames, and buyersFrom() below converts a stored role list once using
 * each account's historical role, so nobody's buying rights changed on the
 * day roles were removed.
 */
export function editVerdict(user, role, settings) {
  ...
  const allowed = Array.isArray(settings && settings.editUsers) ? settings.editUsers : [];
  ...
  if (allowed.map((u) => String(u || "").trim().toLowerCase()).includes(me)) {
    return { allowed: true, why: "Named in PromoPro Settings" };
  }
  return { allowed: false, why: "Not named in PromoPro Settings" };
}

/**
 * ONE-TIME CONVERSION of PromoPro's buyer list, roles to people.
 *
 * `editRoles` named shell roles. Roles are gone, so this expands the stored
 * list into usernames using each account's historical `role` field, which the
 * accounts migration deliberately leaves in place for exactly this.
 *
 * Idempotent: once `editUsers` exists, the old list is never read again.
 * Returns null when there is nothing to convert, so a caller can tell "already
 * done" from "converted".
 */
export function buyersFromRoles(settings, accounts) {
  if (!settings || Array.isArray(settings.editUsers)) return null;
  const roles = Array.isArray(settings.editRoles) ? settings.editRoles : [];
  if (!roles.length) return null;
  const want = new Set(roles.map((r) => String(r || "").trim().toLowerCase()));
  const people = (Array.isArray(accounts) ? accounts : [])
    .filter((u) => want.has(String((u && u.role) || "").trim().toLowerCase()))
    .map((u) => String(u.username || "").trim().toLowerCase())
    .filter(Boolean);

  // A non-empty role list expanding to NOBODY is a lookup that failed, not an
  // answer. Saving it would write an empty buyer list, which means "fall back
  // to shell edit rights" and would silently throw away a setting somebody
  // chose. So: leave it alone and try again next read.
  //
  // This is not hypothetical. The first version of this returned the empty
  // array, and publicUser had stopped carrying the historical role, so every
  // account matched nothing and the list would have been wiped on the first
  // load after deploy.
