...
export function validateSettings(body) {
  ...
  if (b.accountManagers !== undefined) {
    if (!Array.isArray(b.accountManagers)) errors.push("account managers must be a list");
    else {
      const seen = new Set();
      const out = [];
      b.accountManagers.forEach((raw, i) => {
        const a = raw && typeof raw === "object" ? raw : {};
        const name = str(a.name, 120);
        const email = str(a.email, 200);
        if (!name) { errors.push(`account manager ${i + 1}: name is required`); return; }
        if (!looksLikeEmail(email)) { errors.push(`account manager ${i + 1}: email does not look like an address`); return; }
        const id = str(a.id, 100) || name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
        if (seen.has(id)) { errors.push(`two account managers resolve to the same id (${id})`); return; }
        seen.add(id);
        out.push({ id, name, email });
      });
      if (!errors.length) patch.accountManagers = out;
    }
  }
