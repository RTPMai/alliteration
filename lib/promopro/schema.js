 ...
 *
 * `manual: false` means NOBODY TICKS IT. The app sets it, and the screen
 * shows it as a fact rather than as a box to tick:
 *
 *   art sent   the artwork goes out attached to the purchase order itself,
 *              so "have we sent the art" is answered by "have we sent the
 *              PO". It was two ticks for one action, and the second one only
 *              ever got ticked by whoever remembered.
 *   closed     an order is closed when every step before it is done. Asking
 *              somebody to confirm that by hand is asking them to repeat
 *              what the record already says.
 *
 * Both still hold real dates, so nothing about the history or the stage
 * derivation changes. What changed is who is expected to type them.
 * ------------------------------------------------------------------ */
export const STAGES = [
  { key: "draft",       label: "Draft",        dateField: null,            owner: "us",     manual: false },
  { key: "submitted",   label: "Submitted",    dateField: "submittedAt",   owner: "vendor", manual: true },
  { key: "confirmed",   label: "Confirmed",    dateField: "confirmedAt",   owner: "vendor", manual: true },
  { key: "art_approved",label: "Art approved", dateField: "artApprovedAt", owner: "us",     manual: true },
  { key: "paid",        label: "Paid",         dateField: "paymentSentAt", owner: "us",     manual: true },
  { key: "shipped",     label: "Shipped",      dateField: "shippedAt",     owner: "vendor", manual: true },
  { key: "received",    label: "Received",     dateField: "receivedAt",    owner: "us",     manual: true },
  { key: "closed",      label: "Closed",       dateField: "closedAt",      owner: "us",     manual: false },
];

export const STAGE_KEYS = STAGES.map((s) => s.key);

/** The steps a person ticks. The others are the app's job. */
export const MANUAL_STAGES = STAGES.filter((s) => s.manual && s.dateField);

// Every date field a PO carries. artSentAt is no longer a stage of its own
// (the art goes out with the PO) but it is still a real date on the record,
// still patchable, and still shown, so it has to stay in this list or a
// perfectly valid patch would be refused.
export const DATE_FIELDS = STAGES.map((s) => s.dateField).filter(Boolean).concat(["artSentAt"]);
...
/* ------------------------------------------------------------------ *
 * OUTSOURCED WORK, WHICH IS NOT A PURCHASE ORDER
 *
 * Some of what leaves the building never gets a PO raised against it:
 * contract work sent to another shop, finishing, a job farmed out for a
 * week. It still has a vendor, a cost, a clock and a date it has to be
 * back by, which is every reason this app exists, so it lives in the same
 * list and the same pipeline as everything else.
 *
 * ONE FLAG, and it changes exactly three things:
 *   1. Nothing is emailed. There is no document to send, so the send route
 *      refuses one rather than the screen merely hiding the button.
 *   2. The printed sheet says OUTSOURCED JOB, not PURCHASE ORDER, because
 *      handing a vendor paper headed PURCHASE ORDER is raising one.
 *   3. Three step names read differently, since "Submitted" means a PO went
 *      out and nothing went out.
 *
 * Everything else is deliberately identical: same numbering, same stages
 * including payment, same chase clock, same vendor scorecard, same spend.
 * A job sent to a contract shop is a real thing we are waiting on and real
 * money we spent with them, and splitting it into a second kind of record
 * would take it out of both.
 * ------------------------------------------------------------------ */

/**
 * Strict boolean. Anything other than exactly true is a purchase order,
 * because a missing field, a "false" string or an undefined read must never
 * be the thing that stops a PO from being emailed to a vendor.
 */
export function isOutsourced(po) {
  return !!po && po.outsourced === true;
}

/**
 * What a step is called ON THIS RECORD.
 *
 * The stage KEYS never change, so the pipeline lanes, the stored dates, the
 * health rules and every filter carry on working on one set of stages. Only
 * the words differ, and only where the PO wording would be a lie: nothing
 * was submitted, and goods coming back from a contract shop are not being
 * received from a supplier.
 */
export function stageLabel(stageKey, po) {
  const stage = STAGES.find((s) => s.key === stageKey);
  const base = stage ? stage.label : String(stageKey || "");
  if (!isOutsourced(po)) return base;
  const renamed = {
    submitted: "Sent out",
    shipped: "Shipped back",
    received: "Back in house",
  };
  return renamed[stageKey] || base;
}

/**
 * A stage change as a person would say it: "Confirmed to Art approved", not
 * "confirmed to art_approved". Written this way into history now, and
 * readableHistory() tidies lines stored before Sep 24 2026 when they are
 * shown, so old orders read the same as new ones without rewriting records.
 */
function stageWord(key, po) {
  if (key === CANCELLED) return "Cancelled";
  return stageLabel(key, po);
}

export function stageMoveText(before, after, po) {
  return stageWord(before, po) + " to " + stageWord(after, po);
}

const STAGE_KEY_SET = new Set(STAGES.map((s) => s.key).concat([CANCELLED]));

export function readableHistory(what, po) {
  const s = String(what || "");
  const m = s.match(/^([a-z_]+) to ([a-z_]+)$/);
  if (!m || !STAGE_KEY_SET.has(m[1]) || !STAGE_KEY_SET.has(m[2])) return s;
  return stageMoveText(m[1], m[2], po);
}

/**
 * What this record calls itself on paper and on screen. One reader so the
 * printed sheet, the detail header and the tests cannot disagree about
 * whether a thing is a purchase order.
 */
export function docLabels(po) {
  return isOutsourced(po)
    ? { title: "OUTSOURCED JOB", numberLabel: "Job #", noun: "outsourced job" }
    : { title: "PURCHASE ORDER", numberLabel: "Purchase Order #", noun: "purchase order" };
}
 ...
 * OPEN, FINISHED, GONE
 *
 * Three screens and a vendor card were each writing their own version of
 * "not open" as a list of stage names, and the lists had quietly drifted
 * apart: an order that had been checked in was excluded from the pipeline,
 * excluded from the Open list, and included in no other view, so booking a
 * delivery in made the order disappear from everywhere except All. One
 * reader each, so they cannot disagree again.
 *
 * FINISHED IS NOT THE SAME AS CLOSED. closedPatch() only closes an order
 * when every manual step has a date on it, payment included, and payment is
 * entered in QuickBooks rather than here. So in practice orders sit at
 * Received indefinitely, and Received is what "we are done with this" means
 * to the shop floor. Both count as finished.
 * ------------------------------------------------------------------ */

/** The goods are in, or the order closed itself. Nothing is being waited on. */
/**
 * THE NEXT STEP on an order: the first manual stage with no date, and whose
 * move it is. Null when cancelled or when every step is done. The order tab
 * leads with this, so it lives here where it can be tested rather than being
 * worked out inline on the screen.
 */
export function nextStep(po) {
  if (!po || po.cancelledAt) return null;
  const stage = MANUAL_STAGES.find((s) => !po[s.dateField]);
  if (!stage) return null;
  return { stage, onUs: stage.owner === "us" };
}

export function isFinished(po) {
  const s = currentStage(po);
  return s === "received" || s === "closed";
}

/** Still in flight: not finished, not cancelled. */
export function isOpenPo(po) {
  return !isFinished(po) && currentStage(po) !== CANCELLED;
}

/**
 * CLOSED IS DERIVED, NOT TICKED.
 *
 * An order is closed when every step a person ticks has been ticked. Asking
 * somebody to then confirm that by hand is asking them to repeat what the
 * record already says, and the tick that gets forgotten is the one that makes
 * a finished order sit in the pipeline for a month.
 *
 * It runs in both directions on purpose. Unticking a step on an order that
 * closed itself reopens it, because the alternative is an order that says
 * closed with a gap in it and no way to fix that from the screen.
 *
 * The date is the LAST step's date, not today: an order whose final delivery
 * landed on the 4th closed on the 4th, whatever day somebody got round to
 * ticking it.
 *
 * Returns a patch fragment, empty when nothing needs to change, so a caller
 * can merge it without deciding anything.
 */
export function closedPatch(po) {
  if (!po || po.cancelledAt) return {};
  const dates = MANUAL_STAGES.map((s) => po[s.dateField]).filter(Boolean);
  const complete = dates.length === MANUAL_STAGES.length;

  if (complete) {
    const last = dates.slice().sort().pop();
    return po.closedAt === last ? {} : { closedAt: last };
  }
  return po.closedAt ? { closedAt: null } : {};
}
...
/* ------------------------------------------------------------------ *
 * CHASING: THE PART THAT HAPPENS OFF-SCREEN
 *
 * A vendor going quiet raised a colour and a notification from day one, but
 * there was nowhere to say what anybody did about it. Somebody who rang the
 * rep on Tuesday left no mark, so on Thursday the order looked exactly like
 * one nobody had touched in nine days, and the second person rang the same
 * rep about the same order. The trail of what we did was being written to
 * po.history by four routes and read by nothing.
 *
 * A LOGGED FOLLOW-UP DOES NOT STOP THE CLOCK. poHealth()'s silence clock
 * measures VENDOR silence, and us phoning them does not make them less
 * silent. If chasing turned red back to green the order would drop off the
 * board with nothing resolved, which is the exact failure this app exists to
 * end. So the colour is untouched and the chase note sits BESIDE it: "no word
 * for 9 days, chased yesterday" is a different situation from "no word for 9
 * days" and both need to be readable without opening the order.
 * ------------------------------------------------------------------ */

/**
 * How a follow-up happened. `email` is deliberately for mail sent from
 * somebody's OWN inbox: an email sent from inside PromoPro records itself
 * through send.js and never needs typing in.
 */
export const FOLLOW_UP_METHODS = [
  { key: "called", label: "Called them" },
  { key: "voicemail", label: "Left a voicemail" },
  { key: "texted", label: "Texted the rep" },
  { key: "email", label: "Emailed from my own inbox" },
  { key: "other", label: "Something else" },
];

export const FOLLOW_UP_KEYS = FOLLOW_UP_METHODS.map((m) => m.key);

export function followUpLabel(method) {
  const m = FOLLOW_UP_METHODS.find((x) => x.key === method);
  return m ? m.label : "Followed up";
}

/**
 * Validate a logged follow-up. The note is optional: "called them" on its own
 * is a useful fact, and demanding a sentence for it is how logging stops
 * happening. Who and when are stamped by the route, never taken from the
 * caller, same rule as PO numbers and createdBy.
 */
export function validateFollowUp(body) {
  const b = body && typeof body === "object" ? body : {};
  const errors = [];
  const method = str(b.method, 20);
  if (!method) errors.push("say how you followed up");
  else if (!FOLLOW_UP_KEYS.includes(method)) errors.push(`${method} is not a way of following up`);
  const note = str(b.note, 500);
  return { ok: errors.length === 0, errors, followUp: { method, note } };
}

/**
 * The history entry a follow-up becomes. `kind` marks it so the trail can
 * tell what a person did apart from what the app did. Entries written before
 * this existed carry no kind, which reads as "system" and is correct: they
 * were all app-recorded events.
 */
export function followUpEntry(followUp, by, at) {
  const note = str(followUp && followUp.note, 500);
  return {
    at: at || new Date().toISOString(),
    by: String(by || "").toLowerCase(),
    kind: "followup",
    method: str(followUp && followUp.method, 20),
    what: followUpLabel(followUp && followUp.method) + (note ? `: ${note}` : ""),
  };
}

/**
 * When this order was last chased, or null.
 *
 * A re-send counts and the FIRST send does not: the first email is the order
 * itself, not a reminder about it. Reading both means somebody who just
 * presses "Send again" gets credit for it without also having to log a note
 * about the thing the app already recorded.
 */
export function lastChasedAt(po) {
  const logged = po && po.lastFollowUpAt ? String(po.lastFollowUpAt) : "";
  const resent = (po && Number(po.sendCount) > 1 && po.lastSentAt) ? String(po.lastSentAt) : "";
  if (!logged && !resent) return null;
  return logged > resent ? logged : resent;
}

/** "chased 2 days ago", for the line under a health reason. Empty if never. */
export function chaseNote(po, today) {
  const at = lastChasedAt(po);
  if (!at) return "";
  const days = daysBetween(String(at).slice(0, 10), today || new Date().toISOString().slice(0, 10));
  if (days === null) return "";
...
/**
 * What is actually ON this order, for a list that currently only says who we
 * bought it from.
 *
 * The first line's description plus a count of the rest, rather than a
 * concatenation: six descriptions in one table cell is not readable and the
 * first line is what somebody scanning for "the navy hoodies" is matching
 * against. `more` is returned separately so a view can grey it out instead of
 * having it read as part of the product name.
 */
export function productSummary(po) {
  const lines = Array.isArray(po && po.lines) ? po.lines : [];
  const named = lines
    .map((l) => str(l && l.description, 300))
    .filter(Boolean);
  if (!named.length) return { first: "", more: 0, text: "" };
  const first = named[0];
  const more = lines.length - 1;
  return {
    first,
    more: more > 0 ? more : 0,
    text: more > 0 ? `${first} +${more} more` : first,
  };
}
 ...
 *
 * A purchase order that bounced looks, from the pipeline, exactly like a
 * vendor ignoring us: it sits in Submitted going redder every day. It is not
 * the same thing at all. One means chase the rep, the other means nothing
 * ever arrived and the clock has been counting a delivery that never
 * happened. That distinction is the reason this exists; opens are a bonus.
 *
 * WHY "OPENED" IS HEDGED EVERYWHERE BELOW. Open tracking is an invisible
 * image in the message. Apple Mail pre-fetches images on everything it
 * receives and plenty of corporate mail servers do the same, so "opened" can
 * fire with nobody having read a word. Delivered is a fact. Opened is a hint,
 * and it gets worded like one, because "they opened it and still have not
 * replied" is a sentence somebody would act on.
 *
 * NOTHING HERE MOVES A STAGE OR A HEALTH COLOUR. Delivered is not confirmed.
 * Same rule as a logged follow-up and a captured reply: the app records what
 * happened, a person decides what it means.
 * ------------------------------------------------------------------ */

export const DELIVERY_STATES = {
  bounced:   { level: "bad",  label: "Bounced", detail: "This never reached the vendor. Nobody is ignoring it." },
  complained:{ level: "bad",  label: "Marked as spam", detail: "The vendor's mail system flagged this. Assume they have not seen it." },
  failed:    { level: "bad",  label: "Failed to send", detail: "Resend could not send this at all." },
  delivered: { level: "good", label: "Delivered", detail: "Accepted by the vendor's mail server." },
  opened:    { level: "good", label: "Delivered, and opened at least once", detail: "" },
  clicked:   { level: "good", label: "Delivered, and a link was clicked", detail: "" },
  delivery_delayed: { level: "warn", label: "Delayed", detail: "Their mail server is not accepting it yet. Resend keeps retrying." },
  queued:    { level: "warn", label: "Queued", detail: "Handed to Resend, not out of the door yet." },
  sent:      { level: "warn", label: "Sent, no delivery confirmation yet", detail: "Normally a minute or two." },
  scheduled: { level: "warn", label: "Scheduled", detail: "" },
};

/**
 * Turn a raw Resend last-event string into something a person can read.
 *
 * An unrecognised status is reported AS ITSELF rather than swallowed or
 * called unknown. Resend adding a state we have not seen should show up as a
 * strange word on one screen, not as silence that reads like nothing
 * happened.
 */
export function deliveryState(status) {
  const key = String(status || "").toLowerCase().trim().replace(/^email\./, "").replace(/[\s-]+/g, "_");
  if (!key) return null;
  const known = DELIVERY_STATES[key];
  if (known) return { key, ...known };
  return { key, level: "warn", label: key.replace(/_/g, " "), detail: "Resend reported this state and we have no wording for it yet." };
}

/** Opens are only meaningful if tracking was on when the mail went out. */
export function openIsTrusted(state) {
  return !!(state && (state.key === "opened" || state.key === "clicked"));
}

/**
 * Whether asking Resend about this order is worth a call at all.
 *
 * No id means it was sent before we started recording them, or was never
 * sent. Outsourced jobs email nobody. Either way the answer is "do not ask",
 * and saying which of the two it is beats a blank space.
 */
export function deliveryAsk(po) {
  if (isOutsourced(po)) return { ask: false, why: "" };
  if (!po || !po.lastSentAt) return { ask: false, why: "" };
  if (!po.lastMessageId) {
    return { ask: false, why: "This went out before we started recording message ids, so its delivery cannot be looked up." };
  }
  return { ask: true, why: "" };
}
 ...
 * IS THIS LINE PRICED?
 *
 * A unit cost of zero means "nobody has typed a price yet", not "this is
 * free". That is the app's existing convention and not a new one: the
 * Printavo lookup deliberately fills unitCost with 0 and says in a comment
 * that the buyer types it in. Nothing here can express a genuinely
 * no-charge line, and nothing has ever needed to.
 *
 * It exists as a named function because three places have to agree about it:
 * the vendor document, the screen, and the tests. A vendor reading "$0.00"
 * where a price has simply not been agreed yet is being told something
 * untrue, which is the whole reason this was pulled out.
 */
export function linePriced(line) {
  return (Number(line && line.unitCost) || 0) > 0;
}

/** Are any lines priced at all, some, or none? */
export function pricingState(po) {
  const lines = (po && Array.isArray(po.lines)) ? po.lines : [];
  if (!lines.length) return "none";
  const priced = lines.filter(linePriced).length;
  if (priced === 0) return "none";
  if (priced === lines.length) return "all";
  return "some";
}
...
export function validateNew(body, knownVendorIds, knownAmIds) {
  ...
  if (b.printavo && typeof b.printavo === "object") {
    ...
    if (invoiceNumber) {
      printavo = {
        ...
        // Quote or invoice. A snapshot like the names below: a PO raised off
        // a quote was raised before the job was invoiced, and that stays true
        // afterwards. Anything stored before this field existed reads as an
        // invoice, which is what it was, because a quote could not be found
        // at all until Sep 2026.
        kind: str(b.printavo.kind, 20) === "quote" ? "quote" : "invoice",
        customerName: str(b.printavo.customerName, 200),
        // Kept apart from customerName, which is only the one name to SHOW.
        // The vendor's copy of the PO needs the company; "who do I call about
        // this" needs the person. Collapsing them into one field is what put
        // a buyer's personal name where a company belonged.
        //
        // Both are snapshots, like shippingInstructions above: a PO is a
        // document that went to an outside party, and a customer renaming
        // themselves next year must not rewrite what a vendor was told.
        companyName: str(b.printavo.companyName, 200),
        contactName: str(b.printavo.contactName, 200),
  ...
  return {
    ...
    record: {
      ...
      // Outsourced work with no purchase order behind it. Strict true: a
      // malformed body must produce a PO, which is the safe answer, rather
      // than something that quietly refuses to be emailed.
      outsourced: b.outsourced === true,
    },
  };
}
...
export function validatePatch(body, knownVendorIds, knownAmIds) {
  ...
  // Changing your mind about whether a job needed a PO is an ordinary thing:
  // somebody raises one and then the shop just drops the work off. Strict
  // boolean both ways, and the route refuses the flip on anything that has
  // already been emailed, because a document a vendor is holding cannot be
  // un-issued by ticking a box here.
  if (b.outsourced !== undefined) patch.outsourced = b.outsourced === true;
...
export const DEFAULT_SETTINGS = {
  ...
  // ROLES ARE GONE, Sep 2026. This named shell roles; it now names people, by
  // username. lib/promopro/access.js buyersFromRoles() converts a stored role
  // list once, using each account's historical role, so nobody's buying
  // rights changed on the day roles were removed.
  editUsers: [],
  ...
  // AUTOMATIC VENDOR REMINDERS. The outward half of chasing: an email to the
  // vendor when a PO has been sent and no confirmation has come back. OFF
  // until somebody switches it on, because the day it starts working every
  // order already sitting in Submitted becomes eligible at once. The rules,
  // the guards and the reasoning live in lib/promopro/nudge.js.
  nudgeVendors: false,
  nudgeAfterDays: 2,
  nudgeMaxRounds: 2,
  // Orders whose last send is older than this are never reminded about.
  nudgeMaxAgeDays: 30,
  // Sent from the owner rather than an orders mailbox: Ryan's decision, on
  // the grounds that the same sentence carries differently over the name of
  // whoever owns the business. Blank falls back to fromAddress.
  nudgeFromAddress: "",
  nudgeFromName: "",
};
...
export function withSettingDefaults(s) {
  ...
  return {
    ...
    editUsers: Array.isArray(src.editUsers) ? src.editUsers.map((r) => String(r).toLowerCase()) : [],
    // Read through only so the one-time conversion can see it. Never written
    // again; once editUsers exists this is ignored for good.
    ...
    // Strict true only. A missing field, a string, or anything else must
    // never be what starts mail going out to other companies.
    nudgeVendors: src.nudgeVendors === true,
    nudgeAfterDays: Number.isFinite(Number(src.nudgeAfterDays)) && Number(src.nudgeAfterDays) > 0
      ? Math.round(Number(src.nudgeAfterDays))
      : 2,
    nudgeMaxRounds: Number.isFinite(Number(src.nudgeMaxRounds)) && Number(src.nudgeMaxRounds) > 0
      ? Math.round(Number(src.nudgeMaxRounds))
      : 2,
    nudgeMaxAgeDays: Number.isFinite(Number(src.nudgeMaxAgeDays)) && Number(src.nudgeMaxAgeDays) > 0
      ? Math.round(Number(src.nudgeMaxAgeDays))
      : 30,
    nudgeFromAddress: typeof src.nudgeFromAddress === "string" ? src.nudgeFromAddress : "",
    nudgeFromName: typeof src.nudgeFromName === "string" ? src.nudgeFromName : "",
...
export function validateSettings(body, current) {
  ...
  if (b.nudgeVendors !== undefined) patch.nudgeVendors = b.nudgeVendors === true;
  if (b.nudgeFromName !== undefined) patch.nudgeFromName = str(b.nudgeFromName, 120);
  if (b.nudgeFromAddress !== undefined) {
    const v = str(b.nudgeFromAddress, 200);
    if (v && !looksLikeEmail(v)) errors.push("the reminder from-address does not look like an address");
    else patch.nudgeFromAddress = v;
  }

  [
    ["nudgeAfterDays", "reminders must wait at least a day", 1, 30],
    ["nudgeMaxRounds", "there has to be at least one reminder", 1, 5],
    ["nudgeMaxAgeDays", "the reminder cut-off must be at least a day", 1, 365],
  ].forEach(([field, message, min, max]) => {
    if (b[field] === undefined) return;
    const n = Number(b[field]);
    if (!Number.isFinite(n) || n < min) errors.push(message);
    else patch[field] = Math.min(max, Math.round(n));
  });

  // Judged on what the settings will BE, like reply capture above. Switching
  // reminders on with no address to send from would mean a cron that runs
  // every morning, finds work, and silently sends nothing.
  const wantsNudge = b.nudgeVendors !== undefined
    ? b.nudgeVendors === true
    : now.nudgeVendors === true;
  if (wantsNudge) {
    const effectiveFrom = String(
      (b.nudgeFromAddress !== undefined ? patch.nudgeFromAddress : now.nudgeFromAddress) ||
      (b.fromAddress !== undefined ? patch.fromAddress : now.fromAddress) || ""
    ).trim();
    if (!looksLikeEmail(effectiveFrom)) {
      errors.push(
        "set the address reminders come from before switching them on, " +
        "or set the purchase order from-address they fall back to"
      );
    }
  }
  ...
  if (b.editUsers !== undefined) {
    if (!Array.isArray(b.editUsers)) errors.push("the buyer list must be a list");
    else {
      const seen = new Set();
      patch.editUsers = [];
      b.editUsers.forEach((raw) => {
        const r = str(raw, 60).toLowerCase();
        if (!r || seen.has(r)) return;
        seen.add(r);
        patch.editUsers.push(r);
      });
    }
  }
