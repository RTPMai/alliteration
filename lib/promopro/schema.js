...
export function buildPoNumber({ year, invoiceNumber, manualSeq, imprintNumber }) {
  ...
  const base = `${y}-${middle}`;
  // 0 is not a valid imprint number, so a plain falsy check is right here.
  if (!imprintNumber) return base;
  return `${base}-${imprintNumber}`;
}
  ...
  if (b.printavo && typeof b.printavo === "object") {
    ...
    if (invoiceNumber) {
      printavo = {
        ...
        imprintNumber: Number.isFinite(Number(b.printavo.imprintNumber)) && Number(b.printavo.imprintNumber) > 0
          ? Math.round(Number(b.printavo.imprintNumber))
          : null,
        groupId: str(b.printavo.groupId, 100) || null,
        dueDate: null,
      };
