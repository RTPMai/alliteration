...
export async function probeTypes() {
  const out = {};
  for (const typeName of ["LineItem", "LineItemGroup"]) {
    try {
      const data = await gql(`query{__type(name:"${typeName}"){fields{name type{name kind ofType{name kind}}}}}`);
      const fields = (data && data.__type && data.__type.fields) || [];
      out[typeName] = fields.map((f) => ({
        name: f.name,
        type: (f.type && (f.type.name || (f.type.ofType && f.type.ofType.name))) || f.type.kind,
      }));
    } catch (e) {
...
/**
 * Everything on one invoice, using ONLY the field names the probe just
 * confirmed exist. Built dynamically so it cannot fail validation on a name
 * this account does not have.
 */
export async function probeInvoice(id, types) {
  const want = (typeName, candidates) => {
    const have = new Set(((types && types[typeName]) || []).map((f) => f.name));
    return candidates.filter((c) => have.has(c));
  };

  const itemScalars = want("LineItem", [
    "id", "description", "quantity", "color", "sizes", "style", "styleNumber",
    "itemNumber", "sku", "productNumber", "position", "unitCost", "price", "markup",
  ]);
  const groupScalars = want("LineItemGroup", [
    "id", "name", "description", "position", "enabledColumns",
  ]);

  // category is an object on most accounts, so it needs a sub-selection.
  const itemHasCategory = ((types && types.LineItem) || []).some((f) => f.name === "category");
  const itemSel = itemScalars.join(" ") + (itemHasCategory ? " category { id name }" : "");
