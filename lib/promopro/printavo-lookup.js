...
const IMPRINT_FIELD_SETS = [
  { name: "full",    fields: "id name details typeOfWork" },
  { name: "named",   fields: "id name details" },
  { name: "basic",   fields: "id name" },
  { name: "minimal", fields: "id" },
];
...
function imprintText(im) {
  ...
  const name = textOf(firstOf(im, ["name", "label"]));
  const details = textOf(firstOf(im, ["details", "description", "notes"]));
  [type, name, details].forEach((b) => {
...
export async function getInvoice(id) {
  ...
  for (const set of LINE_FIELD_SETS) {
    try {
      ...
      return { invoice, via: set.name, imprintVia: im.via, tried };
    } catch (e) {
