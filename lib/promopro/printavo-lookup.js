...
function invoiceQuery(lineFields, partyFields) {
  return `
...
export async function searchInvoices(term, limit) {
  // Same reasoning as the invoice ladder, smaller: the picker lists jobs by
  // who they are for, and listing them by contact is what sends somebody
  // hunting for a company name that is not on screen. The last rung is the
  // query as it stood, so an account without the nesting still searches.
  const partySets = [
  ...
  let lastError = null;
  for (const party of partySets) {
    const q = `
      query PromoProSearch($q: String, $first: Int) {
        invoices(query: $q, first: $first) {
          nodes { id visualId total customerDueAt status { id name } ${party} }
        }
      }
    `;
    try {
      ...
      return nodes.map((inv) => {
        const company = textOf(firstOf((inv.contact && inv.contact.customer) || {}, ["companyName", "company", "name"]));
        const person = textOf(firstOf(inv.contact || {}, ["fullName", "name"]));
        return {
          id: inv.id ? String(inv.id) : null,
          invoiceNumber: inv.visualId != null ? String(inv.visualId) : null,
      ...
      lastError = e;
      if (!isFieldError(e.message)) throw e;
    }
  }
  throw lastError || new Error("Printavo search failed");
}
 ...
 */
export async function getImprints(id) {
  for (const set of IMPRINT_FIELD_SETS) {
    ...
    try {
      ...
      const out = {};
      nodesOf(data && data.invoice && data.invoice.lineItemGroups).forEach((g) => {
...
export async function getInvoice(id) {
  ...
  // request; the pathological case is six, which only happens on an account
  // whose schema differs from ours and only until the ladder is trimmed.
  for (const party of PARTY_FIELD_SETS) {
    for (const set of LINE_FIELD_SETS) {
      try {
        const data = await gql(invoiceQuery(set.fields, party.fields), { id: String(id) });
        const invoice = normalizeInvoice(data && data.invoice);
        if (!invoice) {
          tried.push({ set: set.name, party: party.name, error: "Printavo returned no invoice for that id" });
          return { invoice: null, via: null, partyVia: null, tried };
        }

        // Best effort. No imprint text is a smaller loss than no invoice.
        const im = await getImprints(id);
        ...
        return { invoice, via: set.name, partyVia: party.name, imprintVia: im.via, tried };
      } catch (e) {
        tried.push({ set: set.name, party: party.name, error: e.message });
        ...
        if (!isFieldError(e.message)) return { invoice: null, via: null, partyVia: null, tried };
      }
    }
  }
