 ...
 * SIZES
 *
 * Printavo keeps one line item per style and color, with the size run
 * hanging off it as `sizes: [{ size: "size_l", count: 15 }, ...]`. A vendor
 * does not ship "100 Gildan tees", they ship 15 L, 30 XL and so on, and the
 * PO has to say so line by line or somebody retypes the run by hand.
 *
 * So one Printavo line with sizes becomes one PO line per size that has a
 * count, in the order a size run is read (infant, toddler, youth, adult,
 * then Other). The size goes in `detail` beside the color, because that is
 * the column the form, the printed PO and the vendor email already show as
 * "Color / sizes". No new field anywhere downstream.
 *
 * Two rules keep the pieces honest:
 *   - A line with no sizes, or only zero counts, stays one line exactly as
 *     before. Promo items (koozies, pens) have no size run.
 *   - If the sizes add up to fewer pieces than the line's total, the
 *     difference becomes its own "size not given" line. Dropping pieces
 *     silently is the one outcome worse than an unsplit line.
 * ------------------------------------------------------------------ */

// Printavo's LineItemSize enum, in the order a size run is read.
const SIZE_ORDER = [
  "6m", "12m", "18m", "24m",
  "2t", "3t", "4t", "5t",
  "yxs", "ys", "ym", "yl", "yxl",
...
const SIZE_LABELS = { other: "Other size" };

/** "size_2xl" -> "2xl". Tolerates case and a missing prefix. */
function sizeKey(raw) {
  return String(raw == null ? "" : raw).trim().toLowerCase().replace(/^size_/, "");
}

/** What a vendor reads: "size_2xl" -> "2XL", "size_other" -> "Other size". */
export function sizeLabel(raw) {
  const key = sizeKey(raw);
  if (!key) return "";
  return SIZE_LABELS[key] || key.toUpperCase();
}

/**
 * The size run on one Printavo line, cleaned and ordered.
 * Returns [{ key, label, count }] with zero and blank counts dropped and
 * repeats of the same size added together.
 */
export function sizeRun(li) {
  const raw = Array.isArray(li && li.sizes) ? li.sizes : nodesOf(li && li.sizes);
  const totals = new Map();
  raw.forEach((s) => {
    if (!s) return;
    const key = sizeKey(s.size);
    const count = Math.round(Number(s.count) || 0);
    if (!key || count <= 0) return;
    totals.set(key, (totals.get(key) || 0) + count);
  });
  const rank = (k) => { const i = SIZE_ORDER.indexOf(k); return i === -1 ? SIZE_ORDER.length - 1 : i; };
  return [...totals.entries()]
    .sort((a, b) => rank(a[0]) - rank(b[0]))
    .map(([key, count]) => ({ key, label: sizeLabel(key), count }));
}

/**
 * One normalized line, split by size. `base` is the line as it would have
 * been before sizes existed; the result is one or more copies of it.
 */
export function splitBySize(base, run) {
  if (!run || !run.length) return [base];
  const color = base.detail || "";
  const withSize = (label) => (color ? color + " / " + label : label);

  const lines = run.map((s) => ({ ...base, qty: s.count, size: s.label, detail: withSize(s.label) }));

  const sized = run.reduce((a, s) => a + s.count, 0);
  const missing = (Number(base.qty) || 0) - sized;
  if (missing > 0) {
    lines.push({ ...base, qty: missing, size: "", detail: withSize("size not given") });
  }
  return lines;
}
...
export function normalizeInvoice(inv) {
  ...
  const groups = rawGroups.map((g, i) => {
    const lines = nodesOf(g.lineItems).map((li) => {
      ...
      // One PO line per size. See SIZES above.
      return splitBySize(base, sizeRun(li));
...
//
// SIZES (Sep 2026). `sizes { size count }` is Printavo's documented
// LineItemSizeCount shape, but unlike the rest of the top rung it has not
// been through the probe on this account. So it sits on its own rung above
// the old top one: if Printavo refuses it, the lookup steps down to
// "no-sizes", which is exactly yesterday's query, and the screen's existing
// "some detail columns are blank" hint names the rung that answered.
const LINE_FIELD_SETS = [
  { name: "full",     fields: "id description itemNumber items color position price merch category { id name } sizes { size count }" },
  { name: "no-sizes", fields: "id description itemNumber items color position price merch category { id name }" },
