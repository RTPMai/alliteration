...
import { poTotal, lineTotal, docLabels, linePriced, pricingState } from "./schema.js";
...
// WHAT AN UNPRICED LINE SAYS ON THE VENDOR'S COPY.
//
// It used to say $0.00, because a blank cost is stored as zero. A purchase
// order can legitimately go out before a price is agreed, and telling a
// vendor in writing that we are paying them nothing for 200 hoodies is worse
// than telling them nothing at all.
//
// Change this one string to change what every copy of the document says: the
// emailed HTML, the printed page and the plain-text part all read it.
const TBC = "to be confirmed";

function cost(line) {
  return linePriced(line) ? money(line.unitCost) : TBC;
}

function extended(line) {
  return linePriced(line) ? money(lineTotal(line)) : TBC;
}

// The total of a PO where some lines are priced and some are not is a real
// number that answers the wrong question, so it is labelled rather than
// presented as the figure. Same rule MarketMachine uses for a rollup with a
// missing week in it: missing is not zero, and a number that quietly treats
// it as zero looks authoritative and is wrong.
function totalText(po) {
  const state = pricingState(po);
  if (state === "all") return money(poTotal(po));
  if (state === "none") return TBC;
  return money(poTotal(po)) + " so far, plus the lines marked " + TBC;
}
...
export function renderPoHtml(po, vendor, opts) {
  ...
  const row = (l) =>
    `<tr>
      ...
      <td style="padding:8px 6px;border-bottom:1px solid ${LINE};text-align:right;vertical-align:top">${cost(l)}</td>
      <td style="padding:8px 6px;border-bottom:1px solid ${LINE};text-align:right;vertical-align:top">${extended(l)}</td>
    </tr>`;
  ...
  <table width="100%" style="border-collapse:collapse;font-size:13px">
    ...
    <tfoot>
      <tr>
        ...
        <td style="padding:10px 6px;text-align:right;font-weight:bold">${totalText(po)}</td>
      </tr>
    </tfoot>
  </table>
...
export function renderEmailText(po, vendor, opts) {
  ...
  lines.forEach((l) => {
    ...
    out.push(`  ${l.qty} x ${cost(l)} = ${extended(l)}`);
  });
  out.push("");
  out.push(`Total: ${totalText(po)}`);
  out.push("");
