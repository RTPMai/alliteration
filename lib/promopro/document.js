...
export function renderPoHtml(po, vendor, opts) {
  ...
  <div style="margin-top:18px;font-size:13px">
    ...
    <div style="color:${MUTED};font-size:12px">Open these links to download. No sign-in needed.</div>
    <ul style="margin:6px 0 0 18px;padding:0">
      ${art.map((a) => `<li style="margin-bottom:3px"><a href="${esc(a.url)}" style="color:${ACCENT}">${esc(a.filename)}</a></li>`).join("")}
    </ul>
  </div>` : ""}
...
export function renderEmailText(po, vendor, opts) {
  ...
  if (art.length) {
    out.push("");
    out.push("Artwork:");
    art.forEach((a) => out.push(`  ${a.filename}  ${a.url}`));
  }
  out.push("------------------------------------------------------------------------");
