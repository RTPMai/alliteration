...
/**
 * The newest few entries, newest first, for the short log on a person's card.
 *
 * The store already hands spends back in date order, but a card showing only
 * the top three would quietly show the WRONG three if that ever changed, so
 * the order is asserted here rather than assumed. A limit of 0 or less means
 * everything.
 *
 * Undated entries sort last. A card has room for three lines and a real
 * purchase deserves them more than one somebody keyed in without a date.
 */
export function recentSpends(spends, limit) {
  const rows = (Array.isArray(spends) ? spends : []).filter(Boolean).slice();
  rows.sort((a, b) => {
