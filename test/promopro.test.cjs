...
(async () => {
  ...
  /* -- imprint numbering: a zero position is a real first imprint -- */

  const groupsAt = (...positions) => positions.map((p, i) => ({
    id: 'g' + i,
    position: p,
    lineItems: { nodes: [{ id: 'l' + i, description: 'Item', items: 1 }] },
  }));

  t.test('a job whose positions start at zero numbers 1 and 2, not 1 and 1', () => {
    // 66883 came back with positions 0 and 1. The old rule read "not greater
    // than zero" as missing, fell back to the array index plus one for the
    // first group and took the second at face value, so both imprints
    // numbered 1 and the PO read 26-66883-1+1.
    t.equal(pl.imprintNumbersFor(groupsAt(0, 1)).join(','), '1,2');
    t.equal(pl.imprintNumbersFor(groupsAt(0, 1, 2)).join(','), '1,2,3');
  });

  t.test('a gap in the positions is kept, because a deleted imprint leaves one', () => {
    // 66608's promo imprint is genuinely numbered 9 on a two-group job.
    // Renumbering it 2 would put the wrong number on a vendor's document.
    t.equal(pl.imprintNumbersFor(groupsAt(1, 9)).join(','), '1,9');
  });

  t.test('two imprints never share a number, whatever Printavo sends', () => {
    // The last line of defence. Whichever way the position data is wrong,
    // two cards reading "Imprint 1" is never the right screen.
    t.equal(pl.imprintNumbersFor(groupsAt(1, 1)).join(','), '1,2');
    t.equal(pl.imprintNumbersFor(groupsAt(0, 0, 0)).join(','), '1,2,3');
  });

  t.test('a job with no positions falls back to the order it arrived in', () => {
    const none = [{ id: 'a', lineItems: { nodes: [] } }, { id: 'b', lineItems: { nodes: [] } }];
    t.equal(pl.imprintNumbersFor(none).join(','), '1,2');
    t.equal(pl.imprintNumbersFor([]).length, 0);
  });

  t.test('one missing position drops the whole job to plain order', () => {
    // Half-trusted ordering is worse than none: numbering the group that
    // does have a position off its position and its neighbour off the array
    // index mixes two different scales on one job.
    const mixed = [
      { id: 'a', position: 5, lineItems: { nodes: [] } },
      { id: 'b', lineItems: { nodes: [] } },
    ];
    t.equal(pl.imprintNumbersFor(mixed).join(','), '1,2');
  });

  t.test('the numbering the picker and the PO number both read is the same one', () => {
    // The card label, the suffix and the stored record all come off
    // group.imprintNumber, so this is the one place it is decided.
    const inv = pl.normalizeInvoice({
      id: 'z', visualId: 66883, contact: { fullName: 'Redwood Builders' },
      lineItemGroups: { nodes: groupsAt(0, 1) },
    });
    t.equal(inv.groups.map((g) => g.imprintNumber).join('+'), '1+2');
    t.equal(s.buildPoNumber({
      year: '26',
      invoiceNumber: inv.invoiceNumber,
      imprintNumber: inv.groups.map((g) => g.imprintNumber).join('+'),
    }), '26-66883-1+2');
  });
